"""Weave span helper for McpToolsetPool emit sites.

Provides a single async context manager ``emit_mcp_pool_span(name, attrs)``
...
Usage::
    ...
    async with emit_mcp_pool_span("mcp_pool.get", {
        "kind": "cloud_run",
        "pool_key": "(cloud_run, my_server, acc, abc123)",
        "cache_hit": cache_hit,
...
McpPoolSpanName = Literal["mcp_pool.get", "mcp_pool.evict"]


@contextlib.asynccontextmanager
async def emit_mcp_pool_span(
    name: McpPoolSpanName,
    attrs: dict[str, Any],
    ...
    Args:
        name: Span operation name — ``"mcp_pool.get"`` or
            ``"mcp_pool.evict"``.
    ...
    try:
        ...
        logger.warning(
            "emit_mcp_pool_span: failed to create Weave call",
            exc_info=True,
        )
        yield
        return
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "emit_mcp_pool_span: failed to finish Weave call",
                exc_info=True,
            )
        # Defensive pop — mirrors sandbox_pool_spans.py and callbacks.py
        # to keep the project-wide pop_call convention.
