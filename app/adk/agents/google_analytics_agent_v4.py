            ...
            for account in result:
                ...
                if "property_summaries" in account:
                    for prop in account["property_summaries"]:
                        output += f"   📈 Property: {prop.get('display_name', 'Unknown')}\n"
...
def get_ga_property_details(tenant_id: str, tenant_credentials: str, property_id: str) -> str:
    """
        ...
        if "rows" in result:
            for row in result["rows"]:
                ...
                if "dimension_values" in row:
                    values.extend([dv.get("value", "") for dv in row["dimension_values"]])
...
3. Run report with activeUsers, sessions for 7daysAgo to today""",
        tools=[list_ga_accounts, get_ga_property_details, run_ga_report, run_ga_realtime_report]
    )
