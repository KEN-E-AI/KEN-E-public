...
class PerformanceComparison:
        ...
        for iteration in range(num_iterations):
            for agent_name in agents:
                ...
                analytics_service.track_agent_execution(
                    ...
                    model="gemini-2.5-flash" if iteration % 2 == 0 else "gemini-2.5-pro",
                ...
                logger.debug(f"{agent_name} iteration {iteration}: {execution_time:.4f}s")
    ...
    def run_comparison(self):
        ...
        logger.info(f"Async events queued: {queue_status.get('metrics', {}).get('events_queued', 0)}")
...
def test_with_mock_agent():
    ...
    logger.info(f"Queue status: {queue_status['metrics']['events_queued']} events queued")
