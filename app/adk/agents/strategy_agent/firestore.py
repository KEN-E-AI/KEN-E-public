...
class FirestoreClient:
    ...
    def __init__(
        ...
        if client:
            ...
            if project_id in numeric_id_mapping:
                ...
                logger.warning(
                    f"Detected numeric project ID {project_id}, mapping to {mapped_id}"
                )
            ...
            if not project_id or project_id not in project_mapping.values():
                ...
                logger.info(
                    f"Using environment-based project ID: {project_id} for environment: {environment}"
                )
    ...
    async def get_best_practices(self, doc_type: str) -> Optional[str]:
        ...
        if not self.db:
            logger.warning(
                "No Firestore client available, using placeholder best practices"
            )
            return json.dumps(
                {
        ...
        try:
            ...
            if doc.exists:
                ...
                logger.info(
                    f"Retrieved best practices for {doc_type} - {len(doc_data)} sections, {len(content)} chars"
                )
                return content
            else:
                logger.warning(
                    f"Best practices not found for {doc_type} at path: strategy_doc_guides/{doc_path}"
                )
                return None
    ...
    async def get_reviewer_guidelines(self, doc_type: str) -> Optional[str]:
        ...
        if not self.db:
            logger.warning(
                "No Firestore client available, using placeholder reviewer guidelines"
            )
        ...
        try:
            ...
            if doc.exists:
                ...
                content = json.dumps(doc_data, indent=2)
                logger.info(
                    f"Retrieved reviewer guidelines for {doc_type} - {len(doc_data)} criteria, {len(content)} chars"
                )
                return content
            else:
                logger.warning(
                    f"Reviewer guidelines not found for {doc_type} at path: strategy_doc_guides/{doc_path}"
                )
                return None
    ...
    def save_strategy_document_sync(
        ...
        """
        logger.info(
            f"[FIRESTORE_SAVE] Starting sync save for {doc_type} - account: {account_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"[FIRESTORE_SAVE] Content keys: {list(content.keys()) if isinstance(content, dict) else 'not a dict'}"
            )
            ...
            logger.info(
                f"[FIRESTORE_SAVE] Successfully saved {doc_type} document for account {account_id}"
            )
    ...
    async def save_strategy_document(
        ...
        """
        logger.info(
            f"[FIRESTORE_SAVE] Starting save for {doc_type} - account: {account_id}"
        )
    ...
    async def get_strategy_document(
        ...
        try:
            ...
            if doc.exists:
                ...
                logger.info(
                    f"No existing {doc_type} document found for account {account_id}"
                )
                return None
    ...
    async def update_strategy_document(
        ...
        try:
            ...
            logger.info(
                f"Updated {doc_type} document for account {account_id} to version {version}"
            )
            return True
    ...
    def _check_and_remove_placeholder_sync(
        self, account_id: str, collection_name: str
    ) -> None:
        """
        ...
        try:
            ...
            if all_docs_exist:
                placeholder_ref = self.db.collection(collection_name).document(
                    "_placeholder"
                )
                if placeholder_ref.get().exists:
                    placeholder_ref.delete()
                    logger.info(
                        f"[FIRESTORE] Removed placeholder document from {collection_name} - all 5 strategy documents complete"
                    )

        except Exception as e:
            logger.warning(
                f"[FIRESTORE] Error checking/removing placeholder for {account_id}: {e}"
            )
            # Don't fail the save operation if placeholder removal fails

    async def _check_and_remove_placeholder_async(
        self, account_id: str, collection_name: str
    ) -> None:
        """
        Check if all 5 strategy documents exist and remove the placeholder if they do.
    ...
    async def get_context(self, account_id: str) -> Optional[StrategyContext]:
        ...
        try:
            ...
            if doc.exists:
                ...
                logger.info(
                    f"No existing strategy context found for account {account_id}"
                )
                return None
...
def extract_field_requirements_from_best_practices(best_practices_json: str) -> str:
    ...
    try:
        ...
        requirements.append(
            "Your response must be ONLY a complete JSON document with ALL these exact fields:"
        )
        requirements.append("")
        ...
        requirements.append("")
        requirements.append(
            "IMPORTANT: Each field MUST follow the EXACT description provided in the BEST PRACTICES section above."
        )
        requirements.append(
            "The BEST PRACTICES section contains the complete requirements for what each field should contain."
        )
        requirements.append(
            "DO NOT skip any fields. DO NOT add extra fields. Output ONLY the JSON document."
        )
...
def extract_validation_criteria_from_guidelines(
    guidelines_json: str, best_practices_json: str
) -> str:
    """
    ...
    try:
        ...
        criteria.append(
            f"1. Check that ALL {len(best_practices)} required fields are present:"
        )
        ...
        criteria.append("")
        criteria.append(
            "2. For EACH field, verify it matches the requirements in REVIEWER GUIDELINES above"
        )
        criteria.append("3. Ensure all content is specific to the company, not generic")
        criteria.append(
            "4. Verify all sections have substantial content (not placeholders)"
        )
        criteria.append("")
