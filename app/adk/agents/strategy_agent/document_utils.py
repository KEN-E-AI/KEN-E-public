...
def _extract_text_from_pdf_impl(pdf_bytes: bytes, filename: str) -> str:
    """
    Internal implementation of PDF text extraction.
    This is the actual extraction logic separated for timeout handling.
    """
    import PyPDF2
    ...
    logger.info(
        f"Extracted {len(full_text)} chars from {filename} ({page_count} pages)"
    )
    return full_text


def extract_text_from_pdf(
    pdf_bytes: bytes, filename: str, timeout: float = 30.0
) -> str:
    """
    ...
    Args:
        ...
        timeout: Maximum time in seconds to wait for extraction (default: 30.0)

    Returns:
        Extracted text from the PDF

    Raises:
        DocumentProcessingError: If PDF extraction fails or times out
    """
    try:
        # Use ThreadPoolExecutor for timeout handling, consistent with existing patterns
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(_extract_text_from_pdf_impl, pdf_bytes, filename)
            return future.result(timeout=timeout)

    except concurrent.futures.TimeoutError as e:
        error_msg = f"PDF extraction timed out after {timeout}s for {filename}"
...
def load_document_from_gcs(
    ...
    try:
        ...
        raise DocumentProcessingError(f"Failed to load {gcs_url}: {e}") from e


def load_documents_from_gcs_urls(urls: list[str], project_id: str) -> dict[str, str]:
    """
...
def create_document_loading_summary(
    loaded_docs: dict[str, str], requested_urls: list[str]
) -> str:
    """
