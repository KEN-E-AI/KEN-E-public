...
class AlertThreshold:
    ...
    def __init__(
        ...
        self.message_template = message_template or (
            "Token usage at {percentage:.1f}% of limit ({tokens:,} / {limit:,} tokens)"
        )
...
class AlertManager:
    ...
    DEFAULT_THRESHOLDS = [
        ...
        AlertThreshold(
            75, AlertSeverity.WARNING, [AlertChannel.LOGGING, AlertChannel.FIRESTORE]
        ),
        AlertThreshold(
            90,
            AlertSeverity.ERROR,
            [AlertChannel.LOGGING, AlertChannel.FIRESTORE, AlertChannel.WEBHOOK],
        ),
        AlertThreshold(
            95,
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "notification_channels": {
                ...
                "webhook": {"enabled": False, "url": None, "headers": {}},
            },
    ...
    def _check_circuit_breaker(
        ...
        if percentage >= self.config.get("circuit_breaker_threshold", 100):
            ...
            logger.critical(
                f"Circuit breaker triggered at {percentage:.1f}% token usage"
            )
    ...
    def _create_and_send_threshold_alert(
        ...
        alert = self._create_alert(
            severity, percentage, current_tokens, max_tokens, None, context, agent_name
        )
        ...
        alert_key = (
            f"{self.account_id}:{threshold_config['percentage']}:{context or 'global'}"
        )
