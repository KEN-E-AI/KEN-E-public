...
class CompetitiveGraphBuilder:
    ...
    def build_competitive_graph(
        ...
        try:
            ...
            logger.info(f"Creating CompetitiveEnvironment node for account {account_id}")
            ...
            substitution_relationships = self._create_substitution_relationships(
                account_id,
                created_nodes['substitute_products']
            )
            created_nodes['may_be_substituted_for_relationships'] = substitution_relationships

            logger.info(f"Successfully created competitive graph with {len(created_nodes['competitors'])} competitors")
    ...
    def _create_competitor_tactic(self, tactic: NamedDetail, competitor_id: str, account_id: str) -> Dict:
        ...
        node = self.neo4j_ops.create_strategy_node('CompetitorTactic', node_data, account_id)
    ...
    def _create_competitor_value_proposition(self, vp: NamedDetail, competitor_id: str, account_id: str) -> Dict:
        ...
        node = self.neo4j_ops.create_strategy_node('ValueProposition', node_data, account_id)
    ...
    def _create_competitor_strength(
        ...
        node = self.neo4j_ops.create_strategy_node('CompetitorStrength', node_data, account_id)
    ...
    def _create_competitor_weakness(
        ...
        node = self.neo4j_ops.create_strategy_node('CompetitorWeakness', node_data, account_id)
    ...
    def _create_substitute_product(
        ...
        node = self.neo4j_ops.create_strategy_node('SubstituteProduct', node_data, account_id)
    ...
    def _create_substitution_relationships(self, account_id: str, substitute_products: List[Dict]) -> List[Dict]:
        """
        ...
        """
        company_products = self.neo4j_ops.connection.execute_query(query, {'account_id': account_id})
        ...
        for company_product in company_products:
            for substitute_product in substitute_products:
                ...
                """
                result = self.neo4j_ops.connection.execute_query(rel_query, {
        ...
        logger.info(f"Created {len(relationships)} MAY_BE_SUBSTITUTED_FOR relationships")
