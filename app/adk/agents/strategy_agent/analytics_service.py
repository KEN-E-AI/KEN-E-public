...
class AnalyticsService:
    ...
    def _init_firestore_clients(self):
        ...
        try:
            ...
            self.default_db = firestore.Client(
                project=self.project_id
            )
    ...
    def track_agent_execution(
        ...
        """
        # Calculate cost
        model_pricing = self.MODEL_PRICING.get(model, self.MODEL_PRICING["gemini-2.5-flash"])
    ...
    def aggregate_daily_costs(self, date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            docs = collection.where(
                filter=FieldFilter("timestamp", ">=", start_time)
            ).where(
                filter=FieldFilter("timestamp", "<", end_time)
            ).stream()
            ...
            # Store aggregation
            agg_collection = self.analytics_db.collection(f"cost_aggregations_{self.account_id}")
            agg_collection.document(start_time.date().isoformat()).set(aggregation)
            
            logger.info(f"Daily cost aggregation for {start_time.date()}: ${aggregation['total_cost']:.2f}")
            return aggregation
    ...
    def get_cost_trends(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            collection = self.analytics_db.collection(f"cost_aggregations_{self.account_id}")
