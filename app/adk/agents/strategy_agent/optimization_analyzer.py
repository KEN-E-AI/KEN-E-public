...
class OptimizationAnalyzer:
    ...
    def _init_firestore_clients(self):
        ...
        try:
            ...
            logger.info(f"Initialized optimization analyzer for account {self.account_id}")
    ...
    def _analyze_peak_usage(
        self, 
        hourly_usage: Dict[int, Dict[str, int]], 
        data: Dict[str, Any]
    ):
    ...
    def _analyze_context_utilization(self, patterns: Dict[str, Any], data: Dict[str, Any]):
        ...
        if prompt_tokens > 0:
            model_config = self.MODEL_CONFIGS.get(model, self.MODEL_CONFIGS["gemini-2.5-flash"])
    ...
    def _check_model_optimization(
        ...
        for model, usage in model_usage.items():
            if "pro" in model.lower():
                avg_tokens = usage["tokens"] / usage["count"] if usage["count"] > 0 else 0
                ...
                if avg_tokens < self.THRESHOLDS["pro_model_simple_task_threshold"]:
                    ...
                    potential_savings = (
                        (pro_cost_per_token - flash_cost_per_token) * usage["tokens"]
                    )
                    savings_percentage = (potential_savings / usage["cost"]) * 100 if usage["cost"] > 0 else 0
    ...
    def _check_error_patterns(
        ...
        for agent_name, agent_pattern in patterns.get("agent_patterns", {}).items():
            error_rate = agent_pattern["errors"] / agent_pattern["executions"] if agent_pattern["executions"] > 0 else 0
            ...
            if error_rate > self.THRESHOLDS["error_rate_threshold"]:
                recommendations.append(OptimizationRecommendation(
                    ...
                    details={
                        ...
                        "wasted_tokens": int(agent_pattern["total_tokens"] * error_rate)
                    }
                ))
    ...
    def _store_recommendations(self, recommendations: List[OptimizationRecommendation]):
        ...
        try:
            ...
            batch.set(doc_ref, {
                ...
                "total_estimated_savings": sum(r.estimated_savings_percentage for r in recommendations)
            })
            ...
            batch.commit()
            logger.info(f"Stored {len(recommendations)} recommendations for account {self.account_id}")
    ...
    def get_latest_recommendations(self) -> List[Dict[str, Any]]:
        ...
        try:
            doc = self.default_db.collection("optimization_recommendations").document(
                f"{self.account_id}_latest"
            ).get()
