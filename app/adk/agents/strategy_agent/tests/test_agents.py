...
class TestAgentCreation:
    ...
    def mock_firestore(self):
        ...
        with patch('app.adk.agents.strategy_agent.agents.get_best_practices_sync') as mock_bp, \
             patch('app.adk.agents.strategy_agent.agents.get_reviewer_guidelines_sync') as mock_rg, \
             patch('app.adk.agents.strategy_agent.agents.extract_field_requirements_from_best_practices') as mock_extract:
            
            mock_bp.return_value = '{"sections": ["overview", "analysis", "recommendations"]}'
            mock_rg.return_value = 'Review for completeness and accuracy'
            mock_extract.return_value = 'Required fields: overview, analysis, recommendations'
    ...
    def test_create_google_search_agent(self):
        ...
        assert agent.description == "Expert web researcher that searches Google for public information"
        assert agent.generate_content_config.temperature == 0.2
        assert agent.generate_content_config.max_output_tokens == 8192
    
    def test_create_business_strategist_with_context(self, mock_context, mock_firestore):
    ...
    def test_create_business_reviewer(self, mock_firestore):
        ...
        assert agent.model == "gemini-2.5-flash"  # Should use flash for cost optimization
        assert agent.tools == [exit_loop]
        assert agent.generate_content_config.temperature == 0.1
        
        mock_firestore['reviewer_guidelines'].assert_called_once_with("business_strategy")
    ...
    @pytest.mark.parametrize("strategist_func,name,doc_type", [
        (create_competitive_strategist, "competitive_strategist", "competitive_strategy"),
    ...
    def test_all_strategists_use_pro_model(self, strategist_func, name, doc_type, mock_context, mock_firestore):
        """Test that all strategist agents use gemini-2.5-pro model."""
        agent = strategist_func(mock_context)
        
        assert agent.name == name
        assert agent.model == "gemini-2.5-pro", f"{name} should use gemini-2.5-pro for quality"
        assert agent.generate_content_config.max_output_tokens == 65535
        
        mock_firestore['best_practices'].assert_called_with(doc_type)
    
    @pytest.mark.parametrize("reviewer_func,name", [
    ...
    def test_all_reviewers_use_flash_model(self, reviewer_func, name, mock_firestore):
        ...
        assert agent.name == name
        assert agent.model == "gemini-2.5-flash", f"{name} should use gemini-2.5-flash for cost"
    
    @pytest.mark.parametrize("editor_func,name", [
    ...
    def test_cascading_document_review_in_instructions(self, mock_context, mock_firestore):
    ...
    def test_firestore_fallback_when_unavailable(self):
        ...
        with patch('app.adk.agents.strategy_agent.agents.get_best_practices_sync', return_value=None):
            agent = create_business_strategist(None)
            
            assert agent is not None
            assert "default best practices" in agent.instruction.lower() or \
    ...
    def test_agent_output_keys(self, mock_context, mock_firestore):
        ...
        for func, expected_key in expected_keys.items():
            ...
            assert agent.output_key == expected_key, f"{func.__name__} should have output_key={expected_key}"