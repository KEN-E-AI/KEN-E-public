...
class TestCriticalAgentFeatures:
    ...
    def test_strategists_use_pro_model(self, mock_extract, mock_best_practices, mock_context):
        ...
        for strategist_func in strategists:
            ...
            assert agent.model == "gemini-2.5-pro", f"{strategist_func.__name__} must use gemini-2.5-pro"
    ...
    def test_reviewers_use_flash_model(self, mock_guidelines):
        ...
        for reviewer_func in reviewers:
            agent = reviewer_func()
            assert agent.model == "gemini-2.5-flash", f"{reviewer_func.__name__} must use gemini-2.5-flash"
    ...
    def test_editors_use_flash_model(self, mock_extract, mock_best_practices):
        ...
        for editor_func in editors:
            agent = editor_func()
            assert agent.model == "gemini-2.5-flash", f"{editor_func.__name__} must use gemini-2.5-flash"
    
    @patch('agents.get_best_practices_sync')
    @patch('agents.extract_field_requirements_from_best_practices')
    def test_cascading_document_review(self, mock_extract, mock_best_practices, mock_context):
        ...
        assert "Review Prior Analysis" in competitive.instruction or "BUSINESS STRATEGY" in competitive.instruction
    ...
    def test_firestore_fallback(self, mock_extract, mock_best_practices, mock_context):
        ...
        assert "Create a comprehensive" in agent.instruction or "default" in agent.instruction.lower()
    
    @patch('agents.get_best_practices_sync')
    @patch('agents.extract_field_requirements_from_best_practices')
    def test_output_keys_for_state_management(self, mock_extract, mock_best_practices, mock_context):
