...
Ensure all required fields are populated."""},
    ...
    async def generate_business_strategy(self, company_name: str, account_id: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            print(f"✅ Structured strategy created with {len(strategy.strategic_goals)} goals")

            # Step 3: Save to Firestore (backup)
            print("\n💾 Step 3: Saving to Firestore...")
            firestore_result = self.save_to_firestore(strategy_dict, "business_strategy", account_id)
            ...
            try:
                ...
                actual_counts = {
                    'business_value_propositions': len(graph_nodes.get('business_value_propositions', [])),
                    'products': len(graph_nodes.get('products', [])),
                    'goals': len(graph_nodes.get('goals', [])),
                    'strengths': len(graph_nodes.get('swot', {}).get('strengths', [])),
                    'weaknesses': len(graph_nodes.get('swot', {}).get('weaknesses', [])),
                    'opportunities': len(graph_nodes.get('swot', {}).get('opportunities', [])),
                ...
                if actual_counts['strengths'] < expected_strengths:
                    print(f"   ⚠️  strengths: Expected {expected_strengths}, got {actual_counts['strengths']}")
                    validation_passed = False
                else:
                    print(f"   ✅ strengths: {actual_counts['strengths']} nodes created")

                if actual_counts['weaknesses'] < expected_weaknesses:
                    print(f"   ⚠️  weaknesses: Expected {expected_weaknesses}, got {actual_counts['weaknesses']}")
                    validation_passed = False
                else:
                    print(f"   ✅ weaknesses: {actual_counts['weaknesses']} nodes created")

                # Opportunities and Risks are dynamic (1-5 per strength/weakness)
                print(f"   ✅ opportunities: {actual_counts['opportunities']} nodes created (linked to strengths)")
                print(f"   ✅ risks: {actual_counts['risks']} nodes created (linked to weaknesses)")
                print(f"   ✅ business_value_propositions: {actual_counts['business_value_propositions']} nodes created")
    ...
    def save_to_firestore(self, strategy_dict: Dict, doc_type: str, account_id: str) -> Dict:
