...
from agents.competitive_agents import (
    ...
    def format_with_openai(self, research_data: str) -> dict[str, Any]:
        ...
        completion = client.beta.chat.completions.parse(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """You are a competitive analysis formatter.
...
Ensure all required fields are populated.""",
                ...
                {
                    "role": "user",
                    "content": f"Format this research into structured competitive analysis:\n\n{research_data}",
                },
            ],
    ...
    async def generate_competitive_analysis(
        ...
        try:
            ...
            print(
                f"✅ Structured analysis created with {len(analysis.competitors)} competitors"
            )

            # Step 3: Save to Firestore (backup)
            print("\n💾 Step 3: Saving to Firestore...")
            firestore_result = self.save_to_firestore(
                analysis_dict, "competitive_analysis", account_id
            )
            ...
            try:
                graph_nodes = self.graph_builder.build_competitive_graph(
                    analysis, account_id, self.user_id
                )

                # Validate graph creation
                expected_counts = {
                    "competitors": len(analysis.competitors),
                    "substitute_products": sum(
                        len(c.substitute_products) for c in analysis.competitors
                    ),
                    "competitor_strengths": sum(
                        len(c.strengths) for c in analysis.competitors
                    ),
                    "competitor_weaknesses": sum(
                        len(c.weaknesses) for c in analysis.competitors
                    ),
                    "competitor_tactics": sum(
                        len(c.marketing_tactics) for c in analysis.competitors
                    ),
                    "competitor_value_propositions": sum(
                        len(c.value_propositions) for c in analysis.competitors
                    ),
                    "substitute_value_propositions": sum(
                        len(c.substitute_products) for c in analysis.competitors
                    ),  # One VP per substitute
                }

                actual_counts = {
                    "competitors": len(graph_nodes.get("competitors", [])),
                    "substitute_products": len(
                        graph_nodes.get("substitute_products", [])
                    ),
                    "competitor_strengths": len(
                        graph_nodes.get("competitor_strengths", [])
                    ),
                    "competitor_weaknesses": len(
                        graph_nodes.get("competitor_weaknesses", [])
                    ),
                    "competitor_tactics": len(
                        graph_nodes.get("competitor_tactics", [])
                    ),
                    "competitor_value_propositions": len(
                        graph_nodes.get("competitor_value_propositions", [])
                    ),
                    "substitute_value_propositions": len(
                        graph_nodes.get("substitute_value_propositions", [])
                    ),
                ...
                print(
                    f"   ✅ risks: {actual_counts['risks']} nodes created (from competitor strengths)"
                )
                print(
                    f"   ✅ opportunities: {actual_counts['opportunities']} nodes created (from competitor weaknesses)"
                )
                ...
                if graph_nodes.get("competitive_environment"):
                    print("   ✅ competitive_environment: 1 node created")
                else:
                    print("   ⚠️  competitive_environment: Missing")
                    validation_passed = False
    ...
    def save_to_firestore(
        self, analysis_dict: dict, doc_type: str, account_id: str
    ) -> dict:
...
async def test_competitive_analysis():
    ...
    try:
        ...
        result = await runner.generate_competitive_analysis(
            company_name="Tesla", account_id="acc_tesla_competitive_test"
        )
