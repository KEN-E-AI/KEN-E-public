...
class TestGCSDocumentLoading:
    ...
    def test_load_text_document_from_gcs(self, mock_storage_client):
        ...
        filename, content = load_document_from_gcs(
            "gs://test-bucket/path/to/test.txt",
            mock_client
        )
    ...
    def test_load_pdf_document_from_gcs(self, mock_storage_client):
        ...
        with patch('app.adk.agents.strategy_agent.document_utils.extract_text_from_pdf') as mock_extract:
            mock_extract.return_value = "Extracted PDF content"
            
            filename, content = load_document_from_gcs(
                "gs://test-bucket/document.pdf",
                mock_client
            )
...
class TestMultipleDocumentLoading:
    ...
    def test_load_multiple_documents_success(self):
        ...
        with patch('app.adk.agents.strategy_agent.document_utils.storage.Client') as mock_storage:
            with patch('app.adk.agents.strategy_agent.document_utils.load_document_from_gcs') as mock_load:
...
class TestDocumentLoadingSummary:
    ...
    def test_summary_partial_loaded(self):
        ...
        requested_urls = ["gs://bucket/doc1.txt", "gs://bucket/doc2.pdf", "gs://bucket/doc3.txt"]
