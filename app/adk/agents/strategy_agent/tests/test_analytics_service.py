...
def test_track_agent_execution(analytics_service, mock_firestore_client):
    ...
    assert metrics["total_cost"] == pytest.approx(expected_prompt_cost + expected_response_cost)
...
def test_cumulative_metrics_update(analytics_service):
    ...
    assert analytics_service.execution_metrics["agent_metrics"]["agent1"]["executions"] == 1
    assert analytics_service.execution_metrics["agent_metrics"]["agent2"]["executions"] == 1
...
def test_get_cost_trends(analytics_service, mock_firestore_client):
    ...
    assert all(trends[i]["date"] <= trends[i+1]["date"] for i in range(len(trends)-1))
