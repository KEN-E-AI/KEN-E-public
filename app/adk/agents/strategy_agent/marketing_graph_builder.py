...
class MarketingGraphBuilder:
    ...
    def build_marketing_graph(
        ...
        try:
            ...
            for product_category in research_report.product_categories:
                logger.info(
                    f"Processing product category: {product_category.category_name}"
                )

                # For each ideal customer profile in this category
                for icp in product_category.ideal_customer_profiles:
                    logger.info(
                        f"Creating customer profile for category: {product_category.category_name}"
                    )
            ...
            logger.info(
                f"Successfully created marketing graph with {len(created_nodes['customer_profiles'])} customer profiles"
            )
    ...
    def _create_customer_profile(
        ...
        node = self.neo4j_ops.create_strategy_node(
            "CustomerProfile", node_data, account_id
        )
        return node_data

    def _create_problem_awareness_strategy(
        self, description: str, customer_profile_id: str, account_id: str
    ) -> dict:
        ...
        self.neo4j_ops.connection.execute_query(
            query, {"strategy_id": node_id, "profile_id": customer_profile_id}
        )
    ...
    def _create_loyalty_strategy(
        ...
        node = self.neo4j_ops.create_strategy_node(
            "LoyaltyStrategy", node_data, account_id
        )
    ...
    def _link_product_category_to_customer_profile(
        self, category_name: str, customer_profile_id: str, account_id: str
    ) -> dict:
        """
        ...
        if result:
            ...
            logger.warning(
                f"ProductCategory '{category_name}' not found for account {account_id}"
            )
            return None
