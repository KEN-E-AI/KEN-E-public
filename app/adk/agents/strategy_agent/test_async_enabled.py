...
def test_async_analytics():
    ...
    try:
        ...
        try:
            from agents.strategy_agent.analytics_helpers import initialize_analytics_services
            ...
            logger.info("✅ Successfully imported analytics_helpers (path modification)")
        ...
        if "AsyncAnalyticsAdapter" in service_type:
            ...
            if result:
                ...
                if "queue_status" in summary:
                    ...
                    logger.info(f"Events queued: {queue_status.get('metrics', {}).get('events_queued', 0)}")
            ...
            logger.info("Make sure the async_analytics_queue.py file is in the correct location")
            return False
