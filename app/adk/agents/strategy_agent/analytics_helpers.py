...
def check_token_limits_before_execution(
    ...
    try:
        ...
        if alert_manager.check_circuit_breaker():
            error_msg = (
                "Circuit breaker open - token limit exceeded. Aborting execution."
            )
            ...
            if performance_profiler and main_operation:
                performance_profiler.end_operation(
                    main_operation, success=False, error=error_msg
                )
...
def report_execution_summary(
    ...
    if performance_profiler and main_operation:
        try:
            ...
            logger.info(
                f"[PERFORMANCE] Operations: {perf_summary.get('total_operations', 0)}"
            )
            logger.info(
                f"[PERFORMANCE] Total duration: {perf_summary.get('total_duration', 0):.2f}s"
            )
    ...
    if analytics_service:
        try:
            ...
            logger.info(f"[ANALYTICS] Total tokens: {exec_summary['total_tokens']:,}")
            logger.info(f"[ANALYTICS] Total cost: ${exec_summary['total_cost']:.4f}")
    ...
    if optimization_analyzer:
        try:
            ...
            if recommendations:
                ...
                logger.info(
                    "[OPTIMIZATION] No optimization recommendations at this time"
                )
