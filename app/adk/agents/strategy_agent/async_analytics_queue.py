...
class AsyncAnalyticsQueue:
    ...
    def _init_firestore(self):
        ...
        try:
            ...
            logger.info(f"Initialized async analytics queue for account {self.account_id}")
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                should_flush = (
                    ...
                    (len(batch) > 0 and self.shutdown_event.is_set())
                )
...
class AsyncAnalyticsAdapter:
    ...
    def track_agent_execution(
        ...
        model_pricing = AnalyticsService.MODEL_PRICING.get(
            model, 
            AnalyticsService.MODEL_PRICING["gemini-2.5-flash"]
        )
