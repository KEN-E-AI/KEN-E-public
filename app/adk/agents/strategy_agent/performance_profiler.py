...
class PerformanceProfiler:
    ...
    def _init_firestore_client(self):
        ...
        try:
            ...
            logger.info(f"Initialized performance profiler for account {self.account_id}")
    ...
    def _store_metrics(self, metrics: PerformanceMetrics):
        ...
        try:
            collection = self.analytics_db.collection(f"performance_profiles_{self.account_id}")
    ...
    def get_bottlenecks(
        ...
        try:
            ...
            query = collection.where(
                ...
                filter=FieldFilter("is_bottleneck", "==", True)
            ).order_by(
                "duration_seconds", direction=firestore.Query.DESCENDING
            ).limit(limit)
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "completed_operations": sum(
                1 for m in self.metrics.values() if m.end_time
            ),
            "failed_operations": sum(
                1 for m in self.metrics.values() if not m.success
            ),
            "total_duration": sum(
    ...
    def analyze_execution_path(self) -> Dict[str, Any]:
        ...
        analysis["critical_path"] = [
            ...
            for op in critical_path_ops if op.duration
        ]
