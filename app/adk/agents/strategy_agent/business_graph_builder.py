...
class GraphBuilder:
    ...
    def build_strategy_graph(
        ...
        try:
            ...
            self._create_business_value_propositions(strategy.business_value_propositions, account_id, created_nodes)
            ...
            logger.info(f"Successfully created strategy graph for {strategy.company_name}")
    ...
    def _create_swot_nodes(self, swot: SWOTAnalysis, account_id: str, created_nodes: Dict):
        ...
        for link in swot.strengths_and_opportunities:
            ...
            created_nodes['swot']['strengths'] = created_nodes['swot'].get('strengths', [])
            ...
            for opp_item in link.linked_opportunities:
                ...
                created_nodes['swot']['opportunities'] = created_nodes['swot'].get('opportunities', [])
        ...
        for link in swot.weaknesses_and_risks:
            ...
            created_nodes['swot']['weaknesses'] = created_nodes['swot'].get('weaknesses', [])
    ...
    def _create_pestel_nodes(self, pestel: PESTELAnalysis, account_id: str, created_nodes: Dict):
        ...
        RETURN p
        """
        pestel_node = self.neo4j_ops.connection.execute_query(pestel_query, {
            'account_id': account_id,
    ...
    def _create_business_model_nodes(self, business_model, account_id: str, created_nodes: Dict):
        ...
        RETURN bm
        """
        bm_node = self.neo4j_ops.connection.execute_query(bm_query, {
            'account_id': account_id,
    ...
    def _create_market_analysis_nodes(self, market_analysis, account_id: str, created_nodes: Dict):
    ...
    def update_strategy_graph(
        ...
        """
        logger.info(f"Updating strategy graph for account {account_id}, version {version.version_number}")
        ...
        for delta in deltas:
            if delta.operation == 'create':
                ...
                SET n.deleted = true,
                    ...
                    n.deleted_by = $user
                """
                self.neo4j_ops.connection.execute_query(delete_query, {
                    'node_id': delta.node_id,
                    'user': delta.changed_by
                })

        logger.info(f"Updated {len(updated_nodes)} nodes for version {version.version_number}")
