...
class StrategyContext:
    ...
    business_strategy: dict | None = None
    competitive_strategy: dict | None = None
    customer_strategy: dict | None = None
    marketing_strategy: dict | None = None
...
def save_strategy_to_firestore(
    context: StrategyContext, doc_type: str, content: dict[str, Any]
) -> bool:
...
def invoke_agent_sync(
    agent: Agent, query: str, user_id: str = None, session_id: str = None
) -> str:
...
def dispatch_to_create_strategy(
    query: str, tenant_context: dict[str, Any] = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Creating strategy for {context.company_name} (account: {context.account_id})"
        )
        ...
        for doc_type, agent in agents.items():
            try:
                ...
                results[doc_type] = f"❌ Error: {e!s}"
                errors.append(f"{doc_type}: {e!s}")
...
"""
        ...
        return {
            "success": False,
            "error": str(e),
            "output": f"Failed to generate strategy documents: {e!s}",
        }


def parse_strategy_context(query: str) -> StrategyContext | None:
...
def app(query: str, tenant_context: dict[str, Any] | None = None, **kwargs) -> str:
    """
