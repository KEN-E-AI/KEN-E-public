...
def delete_reasoning_engine(
    engine_name: str, project_id: str, location: str = "us-central1", force: bool = True
) -> bool:
    ...
    if engine_name.startswith("projects/"):
        ...
        api_path = (
            f"projects/{project_id}/locations/{location}/reasoningEngines/{engine_name}"
        )
    ...
    cmd = ["curl", "-s", "-X", "DELETE", "-H", f"Authorization: Bearer {token}", url]
...
def display_engines(engines: list[dict], keep_id: str | None = None):
...
def delete_engines_with_retry(
    ...
    for i, engine in enumerate(engines_to_delete, 1):
        ...
        result = delete_reasoning_engine(
            engine["name"], project_id, location, force=True
        )
    ...
    if retry_list:
        print_colored(
            f"\n\nRetrying {len(retry_list)} rate-limited deletions...",
            Colors.YELLOW,
            bold=True,
        )
...
Examples:
  ...
  %(prog)s --project my-project --list     List engines in specific project
    ...
    parser.add_argument(
        "--list", "-l", action="store_true", help="List all reasoning engines"
    )

    parser.add_argument(
        "--delete", "-d", action="store_true", help="Delete unused reasoning engines"
    )
    ...
    if args.delete and not args.keep_id and not args.yes and not args.dry_run:
        print_colored(
            "Warning: No --keep-id specified. This will delete ALL engines!",
            Colors.RED,
            bold=True,
        )
    ...
    if args.delete:
        ...
        if not args.yes and not args.dry_run:
            try:
                response = (
                    input(f"\n⚠️  Delete {len(engines_to_delete)} engine(s)? (yes/no): ")
                    .strip()
                    .lower()
                )
                if response != "yes":
        ...
        success, failed = delete_engines_with_retry(
            engines_to_delete, project_id, args.location, args.dry_run
        )
        ...
        if not args.dry_run:
            ...
            if success > 0:
                print_colored(
                    f"✅ Successfully deleted: {success} engine(s)", Colors.GREEN
                )
            if failed > 0:
            ...
            if args.keep_id:
                if len(remaining) == 1:
                    ...
                    if remaining_id == args.keep_id:
                        print_colored(
                            "✅ SUCCESS: Only the specified engine remains!",
                            Colors.GREEN,
                            bold=True,
                        )
                    else:
                        print_colored(
                            f"⚠️  WARNING: Unexpected engine remains: {remaining_id}",
                            Colors.YELLOW,
                        )
                elif len(remaining) > 1:
                    print_colored(
                        f"⚠️  WARNING: {len(remaining)} engines still remain",
                        Colors.YELLOW,
                    )
                else:
                    print_colored(
                        "⚠️  WARNING: No engines remain (including the one to keep)",
                        Colors.YELLOW,
                    )
            else:
                if len(remaining) == 0:
                    print_colored(
                        "✅ SUCCESS: All engines deleted!", Colors.GREEN, bold=True
                    )
                else:
                    print_colored(
                        f"⚠️  WARNING: {len(remaining)} engines still remain",
