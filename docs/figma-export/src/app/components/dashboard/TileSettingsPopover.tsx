...
import type { ViewOverride } from './ArtifactRenderer';
...
// App-palette swatches keyed by CSS variable so dark mode flips automatically.
...
export function TileSettingsPopover({ config, onChange, onClose, align = 'right' }: Props) {
  const isTable = config.viewOverride === 'table';
              ...
              return (
                <button
                  ...
                  className={`size-5 rounded-full border flex items-center justify-center ${
                    active ? 'ring-2 ring-offset-1 ring-[var(--color-border-strong)]' : 'border-transparent'
                  }`}
