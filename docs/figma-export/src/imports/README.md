...
1. **Creation:** A user request to the root agent (`ken_e_agent`) is dispatched to the project-planning specialist, which calls `save_project_plan` → `POST /api/v1/plans/{account_id}` → persisted to Firestore under `accounts/{account_id}/project_plans/{plan_id}`.
...
|----------|--------|-------|--------|
| `/api/v1/plans/{account_id}` | GET | PRD-1 | `list[ProjectPlan]` (filterable by status, campaign, tag) |
...
| `/api/v1/plans/{account_id}/{plan_id}/history` | GET | PRD-1 | audit log entries |
...
|-------------|------|---------|
| `ProjectPlan` / `PlanTask` | `api/src/kene_api/models/project_plan_models.py` | Core Pydantic models; DAG + `launch_time_utc` validators live here |
| `is_system` flag | Same file | Marks platform-owned templates; consumed by Automations PRD-5/6 and Knowledge Graph PRDs |
...
The component's work is split across **6 independently shippable project PRDs** under [`projects/`](./projects/). Split rationale: enable parallel execution by multiple dev teams and shrink the surface area each team holds in context. PRD-6 was added when review of `project-planning-implementation-plan.md` surfaced that nothing in the plan actually fires a task at its scheduled `due_date + launch_time_utc` — KEN-E had no scheduler infrastructure (no Cloud Scheduler, Cloud Tasks, APScheduler), so a dedicated PRD builds it.
...
DM-PRD-05 (Deletion Sweep Rewrite, data-management) ──> PR-PRD-01: Data Model & API  ──┬──> PR-PRD-02: Planning Agent & Tools ─┐
                                                          (BLOCKING — ships first)     ├──> PR-PRD-03: Calendar Page Frontend ─┤
                                                                                       ...
                                                                                       └──> PR-PRD-06: Time-Based Scheduler ────┘    (closes out)
                                                                                               ▲
                                      DM-PRD-00 (Migration Foundation, data-management) ──────┘  (provides project_plans index)
```
...
| 3 | [Calendar Page Frontend](./projects/PR-PRD-03-calendar-page-frontend.md) | Frontend | PR-PRD-01 | 2, 4, 6 | Scheduled |
| 4 | [Event-Driven Orchestrator](./projects/PR-PRD-04-event-driven-orchestrator.md) | Backend | PR-PRD-01 | 2, 3, 6 | Scheduled |
| 5 | [Integration Testing & Polish](./projects/PR-PRD-05-integration-testing-and-polish.md) | QA + first-finished team | PR-PRDs 1–4, 6 | — | Scheduled |
| 6 | [Time-Based Scheduler](./projects/PR-PRD-06-time-based-scheduler.md) | Backend / Infra | PR-PRD-01, DM-PRD-00 | 2, 3, 4 | Scheduled |

### Recommended workflow

1. **Data-management phase** (see [`../data-management/README.md`](../data-management/README.md) §5.3): DM-PRD-00 → DM-PRD-01/02/03/04 in parallel → DM-PRD-05. Expected ≈ 7–10 working days with 3–4 teams.
2. **Once DM-PRD-05 merges:** Backend team ships PR-PRD-01. Other teams stay engaged by reviewing the data contract and stubbing their own work against the published Pydantic schema. DM-PRD-00 should already be merged by this point; if not, PR-PRD-06 stays parked until it is.
3. **Once PR-PRD-01 merges:** kick off PR-PRDs 2, 3, 4, and 6 in parallel — four teams, no blocking dependencies between them. Coordinate only on the small integration touchpoint between PR-PRD-03 and PR-PRD-04 (the notification-deep-link snippet in `NotificationSidebar.tsx`).
4. **Closing sprint:** PR-PRD-05 closes out with end-to-end testing once everything else is merged.
...
| [`../agentic-harness/README.md`](../agentic-harness/README.md) | §2 Architecture, §2.4 Key Abstractions | When adding the `dispatch_to_project_planning` handler and registry entry in PRD-2. |
| [`../agentic-harness/projects/AH-PRD-02-agent-factory.md`](../agentic-harness/projects/AH-PRD-02-agent-factory.md) | §5.2 Config-to-constructor mapping | Firestore fields that define an agent config, consumed by the factory when PRD-2 writes `agent_configs/project_planning`. |
...
- `is_system=True` marks platform-owned templates: filtered out of the automations list page, read-only on the details page, enforced by downstream PRDs (A-PRD-5 / A-PRD-6) and not by PRD-1.
...
- `accounts/{account_id}/project_plan_audit/{audit_id}` — audit entries
- Cross-account queries (PRD-6 scheduler) use a **collection-group** query over `project_plans`, not per-account iteration.
