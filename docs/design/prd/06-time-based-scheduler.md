...
**Blocked by:** PRD-1 (needs `launched_at` field in `PlanTask` schema)
...
- **PRD-1:** must include `launched_at: datetime | None` on `PlanTask` from v1 (coordinated in PRD-1's data contract)
- **PRD-4:** exposes `TaskOrchestrator.on_task_due(account_id, plan_id, task_id) -> OrchestratorResult`
- **External:** Cloud Scheduler (GCP), Firestore composite index
...
| Create | `deployment/terraform/firestore_indexes.tf` (or modify existing) — composite index `(status ASC, due_date ASC, launch_time_utc ASC)` per `project_plans_*` collection group |
...
| Composite index not created → query falls back to scan | Index is part of the Terraform PR; CI verifies presence |
| Firestore collection-group queries require the `_account_id_` to be in path — confirm syntax | Spike during the first day; if blocking, fall back to per-account iteration (`for account in active_accounts: query project_plans_{account_id}`). Acceptable for moderate account counts. |
...
- Cross-PRD coordination: [PRD-1 §Data contract](./01-data-model-and-api.md#4-data-contract) (`launched_at` field), [PRD-4 §Data contract](./04-event-driven-orchestrator.md#4-data-contract) (`on_task_due` signature)
