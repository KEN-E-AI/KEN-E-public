...
> **Note on scope:** This PRD covers **event-driven** orchestration only. Time-based triggering of tasks at `due_date + launch_time_utc` is owned by [PRD-6](./06-time-based-scheduler.md) — when the scheduler fires a task, it hands off to the orchestrator defined here.
