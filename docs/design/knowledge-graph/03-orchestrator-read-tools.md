# KG-PRD-3 — Orchestrator Read Tools (Hierarchical Context + Hybrid Search)

**Status:** Ready for development (after KG-PRDs 1 + 2 merge)
**Owner team:** Agent / ML
**Blocked by:** KG-PRD-1, KG-PRD-2
**Parallel with:** KG-PRD-5
...
### Out of scope
- Write tools of any kind (KG-PRD-4 owns the session-end writer).
...
- **KG-PRD-1:** `kb_vector_index`, `:KGNode` label, `kg_node_account_id` index.
- **KG-PRD-2:** `Observation` node type + `source_session_id` property (used by `list_observations`).
- **External:**
...
- Harness design: `docs/KEN-E-Agentic-Harness-Design.md` §3.2 (Hierarchical Context Loading).
