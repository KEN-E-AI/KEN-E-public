# KG-PRD-5 — Research-on-Creation Refactor (Strategy Agents → GraphSyncService + `ResearchRun`)

**Status:** Ready for development (after KG-PRDs 1 + 2 merge)
**Owner team:** Agent / ML
**Blocked by:** KG-PRD-1, KG-PRD-2
**Parallel with:** KG-PRD-3, KG-PRD-4
...
- **KG-PRD-1:** `:KGNode` label and `kg_node_id_unique` constraint. Without these, the per-node uniqueness check on create has to rely on per-label constraints (which don't exist).
- **KG-PRD-2:** `ResearchRun` node type, `create_research_run` / `close_research_run`, `:ESTABLISHED_BY` edge handling in `create_node`, `source_research_run_id` property.
- **External:** none new.
...
The `ResearchRun` node itself (from KG-PRD-2):
...
4. Firestore is kept in sync: every node that appears in Neo4j also appears in its corresponding `strategy_docs_{account_id}` collection. Count match within ±0 after a full research run.
...
- KG-PRD-1 (shared label, constraints); KG-PRD-2 (ResearchRun + provenance methods).
