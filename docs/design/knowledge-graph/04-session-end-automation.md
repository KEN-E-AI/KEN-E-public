# KG-PRD-4 — Session-End as a System-Triggered Automation
...
**Parallel with:** KG-PRD-5 (same dev can ship both if scheduled)
...
- Two ADK agents in `app/adk/agents/session_end_agent/`:
  - `reviewer.py` — LLM agent; reads transcript + KG via KG-PRD-3 tools; emits a `SessionReview` Pydantic payload; saves it as a `TaskArtifact` via A-PRD-3's `attach_task_artifact` tool.
  - `applier.py` — deterministic Python wrapped as an agent task; downloads the artifact, routes each proposed change by autonomy rules, applies via `GraphSyncService` (KG-PRD-2) or halts the task for HITL.
...
**Hard blockers:**
- **KG-PRD-1:** `:KGNode`, constraints, vector index.
- **KG-PRD-2:** `Session` lifecycle (`touch_session`, `close_session`), `Observation` / `create_observation` / `supersede_observation`, provenance stamping.
- **KG-PRD-3:** read tools (`load_context_section`, `load_document`, `search_kb`, `list_observations`) that the reviewer consumes.
...
- KG-PRD-1, 2, 3 (direct dependencies).
