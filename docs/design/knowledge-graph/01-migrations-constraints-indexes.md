# KG-PRD-1 — Neo4j Migrations, Constraints, Indexes, `:KGNode` Backfill
...
### Out of scope
- New node types (`Session`, `Observation`, `ResearchRun`) — KG-PRD-2 owns those
- Any changes to `GraphSyncService` methods — KG-PRD-2
- Tools that query the vector index — KG-PRD-3
- Changing how strategy agents write — KG-PRD-5
...
| Neo4j version < 5.13 in some env (no vector index) | Verify versions before kickoff; upgrade ticket first. Vector index is not optional — KG-PRD-3 depends on it. |
