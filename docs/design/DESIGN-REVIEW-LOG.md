...
1. **Approved: Per-Turn Dispatch Agent (AH-PRD-09).** Deployed root becomes a thin dispatcher whose only tool is `delegate_to_specialist(name, query, acceptance_criteria=None)`. Specialists are resolved per turn from Firestore via a new `specialist_runtime` module (TTL + content-hash cache with per-key striped locking, LRU 256). `_REDEPLOY_REQUIRED_FIELDS` shrinks to the empty set for specialists; `AgentConfigUpdateResponse.warnings` was initially described as vestigial [**Correction AH-81:** AH-60 Finding #1 reversed this — the field is load-bearing for root-agent redeploy signals; see AH-PRD-09 §7 AC #24]. Admin agent edits propagate to the next chat turn within ~60 s without redeploy.
...
| `AgentConfigUpdateResponse.warnings: list[str]` API field | Initially marked vestigial in Phase 2. **[Correction AH-81 2026-06-03: AH-60 Finding #1 reversed this — the field is load-bearing as the sole admin signal that root-agent `model`/`temperature`/`max_output_tokens`/`thinking_budget` edits require a redeploy. Removal declined; see AH-PRD-09 §7 AC #24.]** |
...
6. **~~`MergedAgentConfig.warnings` field removal~~** — **[Cancelled by AH-60 Finding #1; resolved in AH-81 (2026-06-03, Option A). The field (`AgentConfigUpdateResponse.warnings`) is retained as load-bearing; see AH-PRD-09 §7 AC #24.]**
...
4. **`AgentConfigUpdateResponse.warnings` originally scheduled for R2 removal (AH-81).** AH-60 emptied `_REDEPLOY_REQUIRED_FIELDS` for specialists so the field returns empty for specialist edits. [**Superseded by AH-60 Finding #1; resolved in AH-81 (2026-06-03) with Option A — removal declined. The field is load-bearing: `_REDEPLOY_REQUIRED_FIELDS` is non-empty for the root agent (`model`/`temperature`/`max_output_tokens`/`thinking_budget`), making this the sole admin-visible redeploy signal for root edits. See AH-PRD-09 §7 AC #24.**]
