...
**Date:** 2026-04-27
**Stack:** PR #260 → #262 → #263 → #265 → #269 (closeout: PR for this review)
...
3. **OTEL probe never closed (latent inconsistency).** Sprint 5 commit `8360b02` re-enabled Agent Engine tracing AND commented out `OTEL_PYTHON_DISABLED_INSTRUMENTATIONS=google-genai` in `.env.development` + `deploy_ken_e.py` "to test if bug still triggers on ADK 1.26.0" — but the diagnostic was never run. `.env.staging` + `.env.production` kept the workaround active, so dev/staging+prod were inconsistent for ~2 months. Sprint 6 closed the probe (Outcome B, clean run) and stripped the workaround consistently.
...
- `app/adk/.env.development`, `.env.staging`, `.env.production`, `app/adk/deploy_ken_e.py` — OTEL workaround removed everywhere.
- `docs/spike-otel-pydantic-findings.md` — Sprint 6 verification note + status → "Workaround removed".
