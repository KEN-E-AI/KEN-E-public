...
> **Status (2026-04-19):** This plan is split into 5 independently shippable PRDs in [`prd/`](./prd/) for parallel execution by multiple dev teams. See [`prd/README.md`](./prd/README.md) for the dependency graph and team workflow.
>
> **Prerequisite:** Feature 2.2 (Agent Factory — Sprint 9) must ship the **forward-compat schema fields** (`skill_ids`, `sandbox_code_executor_enabled`) before Sprint 2.6-B (Agent Integration) can start. All other Skills PRDs can run in parallel with Sprint 9.
...
Feature 2.6 gives end-users a way to **capture and reuse their domain expertise** as modular, agent-loadable Skills. A user authors a SKILL.md file (with optional reference files and executable scripts) in the Workflows UI, saves it, and attaches it to a custom specialist agent in the Agent Builder (delivered by Feature 2.2 — Agent Factory). When that agent runs, the ADK's `SkillToolset` auto-generates discovery tools (`list_skills`, `load_skill`, `load_skill_resource`) so the agent loads only the skill it needs, when it needs it.
...
| 5 | **Agent placement** | Root agent has system-owned skills users cannot modify. User-authored skills may only be attached to **user-created custom specialist agents** (per Feature 2.2 §8.7). |
...
Full detail in [PRD 01 — Skills Backend](./prd/01-skills-backend.md). Headlines:
...
**Agent config extensions (delivered by Sprint 9 forward-compat):**
...
The agent-config endpoints (`/api/v1/accounts/{account_id}/agent-configs/*`) are delivered by Feature 2.2 (Sprint 9, story 2.2-8). Sprint 2.6-D extends them to accept `skill_ids` and `sandbox_code_executor_enabled` in request/response bodies.
...
|---|---|---|---|---|
| **2.6-0** | Sandbox Spike | [00](./prd/00-sandbox-spike.md) | Platform + Security | 3–5 days |
| **2.6-A** | Skills Backend — Storage, API, Loader | [01](./prd/01-skills-backend.md) | Backend | 6–8 days |
| **2.6-B** | Agent Factory — Skills & Sandbox Integration | [02](./prd/02-agent-integration.md) | Agent Platform | 5–7 days |
| **2.6-C** | Skills Authoring UI | [03](./prd/03-authoring-ui.md) | Frontend | 6–8 days |
| **2.6-D** | Agent Builder Controls + E2E | [04](./prd/04-agent-builder-controls.md) | Frontend + Backend | 4–5 days |

## 7. Dependencies on Feature 2.2 (Sprint 9 — Agent Factory)

Sprint 9 must absorb two small, additive schema changes **before Sprint 2.6-B can start**. Neither requires new UI or endpoint behavior from Sprint 9 — they are data-layer placeholders that Sprint 2.6-B, 2.6-C, and 2.6-D then light up.
...
- Parent feature — Feature 2.2 (Agent Factory) — Notion Sprint 9 page
- Sibling docs — [`agent-hierarchy.md`](../agent-hierarchy.md) §5.1 (overlay model), §8 (config-to-constructor mapping)
