...
4. **Orchestrator reads (KG-PRD-03):** When the user asks about the account, the root delegates via `delegate_to_specialist` (AH-PRD-09 Phase 2) to a specialist, which picks one of four read tools: `load_context_section(section)` for a whole domain (products / icps / competitors / strategies / brand), `load_document(entity_type, entity_id)` for drill-down, `search_kb(query)` for semantic fallback via the vector index, `list_observations(subject?)` for long-tail conversational facts. The four tools are registered in `app/adk/tools/registry/tools.yaml` under `function_tools:` with `default_global: true`, so they surface on every runtime-resolved specialist via AH-PRD-06 PR-C (deploy-time factory) + AH-PRD-09 Phase 3 (runtime resolver) injection — same pattern as `create_visualization` and the system skill mechanism. All four are account-scoped by `tool_context.state["account_id"]` (never by LLM argument). Calendar / campaign / performance data lives in Firestore (Project Tasks `Campaign`) and SAR-E — not in Neo4j — so it is out of scope for these read tools; the orchestrator reads it through component-specific paths.
...
| `kb_read_tools` (`load_context_section` / `load_document` / `search_kb` / `list_observations`) | `app/adk/tools/registry/tools.yaml` (`default_global: true`); functions in `app/adk/agents/shared_tools/kb_read_tools.py` | Four read primitives surfaced on every runtime-resolved specialist via AH-PRD-06 PR-C / AH-PRD-09 Phase 3 `default_global` injection — account-scoped via `tool_context.state["account_id"]`; LLM-supplied account_id always ignored. Not on the root (which carries only `delegate_to_specialist` post-AH-PRD-09). |
...
|-----------|------------|
| Specialists (factory-built + runtime-resolved) | Receive the four KG-PRD-03 read tools automatically via `default_global: true` injection (AH-PRD-06 PR-C / AH-PRD-09 Phase 3). Replaces the partial keyword-detection path (`SECTION_KEYWORDS`, `should_load_section`) per the Context Management decision ([Review 4 in DESIGN-REVIEW-LOG](../../DESIGN-REVIEW-LOG.md#review-4-context-loading--keyword-detection--agent-driven-loading)). The root carries only `delegate_to_specialist` post-AH-PRD-09 Phase 2 — KB tools live on the specialists that actually consume the knowledge. |
