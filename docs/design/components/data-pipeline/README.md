...
| `api/src/kene_api/routers/internal/data_pipeline_catalog.py` | OIDC-authed internal catalog reads — `GET /api/v1/internal/data-pipeline/jobs`, `GET .../jobs/{job_id}`, `GET .../jobs/{job_id}/history-depth?account_id=...`. Service-to-service surface for SAR-E. DP-PRD-01. |
...
| `frontend/src/components/dataPipeline/PipelineInputsForm.tsx` | JSON-Schema → form renderer for job inputs (inline validation, primitive subset per DP-PRD-04 §2). DP-PRD-04. |
| `frontend/src/components/dataPipeline/PipelineRunPanel.tsx` | Run viewer inside `ActivityDetailPanel` — status, resolved inputs, artifact preview, cache indicator, Weave link. DP-PRD-04. |
| `frontend/src/components/dataPipeline/PipelineArtifactPreview.tsx` | First-N-row table for Parquet / JSON artifacts. DP-PRD-04. |
| `frontend/src/components/dataPipeline/CustomJobAuthoringPanel.tsx` | Inline 4-step authoring panel (Basics → Schemas → Connection → Preview → Publish & Use), mounted in the same side-panel — no standalone route. DP-PRD-04. |
...
| `/api/v1/internal/data-pipeline/jobs/{job_id}/history-depth` | GET | DP-PRD-01 (per-connector implementations in DP-PRD-02 / DP-PRD-05) | OIDC; `?account_id=...` → `{"weeks_available": int \| null}`. Dispatches via `connector.get_history_depth(credentials)`. `409` if the connection is in `needs_reauth`. |
...
|---|---|---|
| `GET /api/v1/internal/integrations/credentials/{account_id}/{platform_id}` | IN-PRD-01 (refresh by IN-PRD-02) | Every connector's per-run credential fetch — OIDC-authed, returns `{access_token, expires_at, external_account_id}` per IN-PRD-01's v1 contract. Mailchimp also requires the `dc` prefix surfaced from this endpoint or a sibling — see DP-PRD-05 §5.3 open question. |
...
| **Automations — A-PRD-03 (Task Artifact System)** | Pipeline output is written through A-PRD-03's `artifact_store.create()` non-agent creation path (`created_by="data_pipeline:{job_id}"`) into `gs://kene-task-artifacts-{env}/…` with 30-day lifecycle + 100 MB cap; no parallel storage path. The agent-side `attach_task_artifact()` ADK tool is untouched. | [A-PRD-03](../automations/projects/A-PRD-03-task-artifact-system.md) |
...
|-----------|------------|
| **SAR-E (SE-PRD-01 — Configuration Foundation, SE-PRD-02 — Weekly KPI ingestion)** | Hard dependency on three surfaces: (a) the four daily GA jobs seeded by DP-PRD-02 produce the `[{date, value}]` time-series rows that feed SAR-E's VAR model — daily `is_system` automation runs 4 pipeline tasks + 1 `sar-e.ingest` agent task, stable output schema across runs is a contract; (b) the OIDC-authed `GET /api/v1/internal/data-pipeline/jobs?account_id=...` (DP-PRD-01 §6.7) drives SE-PRD-01's KPI-source validator + `/config/status` listing; (c) the OIDC-authed `GET /api/v1/internal/data-pipeline/jobs/{job_id}/history-depth?account_id=...` (DP-PRD-01 §6.7 + DP-PRD-02's GA implementation) drives SE-PRD-02's backfill-plan probe. |
...
```
  ...
  IN-PRD-02 (Google OAuth + credentials) ─┤
                                          ▼
                                    ┌────────────────┐
                                    │   DP-PRD-01    │  Foundation (models + service + cache + internal catalog/history-depth)
                                    └──┬─────────────┘
                                       │
                                       ▼
                                    ┌────────────────┐
                                    │   DP-PRD-02    │  Google Analytics connector + 8-job catalog + GA history-depth
                                    └──┬────┬────────┘
                          ┌────────────┘    │
                          ▼                 │
  PR-PRD-04 ──┐                             │       ┌──────────────────────┐
  ...
  A-PRD-04  ──┼───────►   DP-PRD-03         │       │   (SAR-E ingestion)  │
         ...
         └──┬─────────┘                     │
            │                               │
            ▼                               ▼
  A-PRD-06 ────┐                    DP-PRD-05 ◀── IN-PRD-04
  IN-PRD-03 ───┼───►   DP-PRD-04    Additional
  PR-PRD-03 ───┘       Frontend +    connectors
                       custom jobs
                          │               │
                          └───────┬───────┘
                                  ▼
...
| 04 | [Frontend + Custom-job Authoring](./projects/DP-PRD-04-frontend-and-custom-jobs.md) | Frontend | DP-PRD-03, A-PRD-06, IN-PRD-03, PR-PRD-03 | — | 5–6 days |
