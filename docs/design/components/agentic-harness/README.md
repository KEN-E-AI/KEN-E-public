...
> 
> **Supervisor-orchestration successor (AH-PRD-05, target architecture, AH-99 GO-confirmed 2026-06-01):** For multi-task turns, a `LlmAgent(mode='chat')` coordinator decomposes the user message into a TODO ledger, delegates per-task via `mode='task'` specialists (call-and-return), and fans out via `ctx.run_node` + `asyncio.gather`. Both paths preserve Billing/Chat/MER-E parity natively. Implementation gate: the ADK 2.0 Foundation PRD ([AH-PRD-13](./projects/AH-PRD-13-adk2-foundation.md)). Full spec: [AH-PRD-05](./projects/AH-PRD-05-multi-step-workflows.md).
