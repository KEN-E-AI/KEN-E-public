...
7. **ADK 2.0 migration chain — after AH-PRD-09 ships (the runtime baseline it migrates).** AH-PRD-13 (Foundation — unpin to 2.0 + breaking-change audit + deploy-tree decoupling, opening with a Phase-0 de-risking spike) → AH-PRD-14 (Contract Preservation — outer-stream / trace hardening, mostly on 1.34.1) → AH-PRD-15 (AgentTool → task-mode migration; owns the 2.0 prod-cutover go/no-go). The chain gates AH-PRD-05.
