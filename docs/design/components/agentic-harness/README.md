...
> 
> **Supervisor-orchestration successor (AH-PRD-05) — shipped & live in dev (AH-160 + AH-161, PR #958, 2026-06-09).** For multi-task turns, a `LlmAgent(mode='chat')` coordinator decomposes the user message into a `supervisor_ledger`, delegates per-task via `mode='task'` specialists (call-and-return, dispatched by the **bare specialist `doc_id`** — `request_task_<name>` is a myth), and fans out via `ctx.run_node`. Both paths preserve Billing/Chat/MER-E parity natively. Four non-obvious ADK constraints govern the dispatch — request_task-only (transfer and task are mutually exclusive per instance); never review-wrap a task specialist (a review `LoopAgent` returns `{"output": None}`); results surface only via `finish_task`; deploy-time-seed dispatch tools because ADK snapshots `tools_dict` before the per-turn attach callback — see [`mcp-architecture.md` §"ADK 2.0 … constraints" #7–#9](./mcp-architecture.md) and [AH-PRD-05 §0](./projects/AH-PRD-05-multi-step-workflows.md). Built on the ADK 2.0 Foundation PRD ([AH-PRD-13](./projects/AH-PRD-13-adk2-foundation.md)).
...
| `app/adk/agents/google_analytics_agent_v4.py` | **[DEPRECATED 2026-06-01]** — R1.0 hand-wired GA agent superseded by `agent_configs/google_analytics_specialist` (AH-25 / AH-PRD-03 Phase 1). The root now reaches GA via `transfer_to_agent("google_analytics_specialist")` resolved per turn by `specialist_runtime`. Target removal: 2026-09-01, once AH-PRD-09 Phase 5 removes the legacy callers in `dispatch_handlers.py` and the test suite. |
...
6. **Multi-task supervisor-orchestrated turn (target, AH-PRD-05):** For a multi-specialist request, the `mode='chat'` coordinator decomposes the user message into a TODO ledger in `session.state`. For each ready task (per `depends_on` DAG), it delegates to a `mode='task'` specialist via ADK's delegation primitive; the specialist runs, writes its result to `{result_key}` in session state, and returns control to the coordinator. For independent tasks, the coordinator fans out via `ctx.run_node` + `asyncio.gather`. The coordinator synthesizes the per-task results and returns a unified response. The review loop (`LoopAgent` leaf) wraps each per-task delegation as today. All inner `usage_metadata` reaches the outer stream natively (AH-99 probe-1 + probe-4).
...
**Supervisor-orchestrated multi-task routing (AH-PRD-05 target):** When a turn requires multiple specialists, the `mode='chat'` coordinator builds a TODO ledger and drives per-task delegation from the ledger — each task specifies an `assignee` (specialist doc_id), `query`, and `criteria`. Routing per task remains description-based (the coordinator picks the right specialist from the Available Specialists block); what changes is that multiple tasks can be dispatched in the same turn and results synthesized before responding to the user. Single-specialist queries continue to use `transfer_to_agent` as today.
...
The component's work is split across **15 project PRDs** (plus the small AH-PRD-02b follow-up, which has no standalone PRD file) under [`projects/`](./projects/). AH-PRD-01 (Review Loop) and AH-PRD-02 (Agent Factory) form the canonical Release 1 base; AH-PRD-09 (Per-Turn Dispatch Agent) supersedes AH-PRD-02's deploy-time path with a runtime resolver in the same release; AH-PRD-03 (GA Specialist) lands on top of all three because the runtime resolver (`specialist_runtime` from AH-PRD-09 Phase 2) plus pooled MCP toolsets with the `kind="cloud_run"` schema field (AH-PRD-09 Phase 3) are how the GA specialist gets constructed per turn. AH-PRD-06 (Per-Agent Tool Mapping) layers individual-tool selection onto the agent factory built by AH-PRD-02, replacing today's coarse server-level attachment; AH-PRD-06 shipped in R1 across three PRs (PR-A #472 backend, PR-B #473 frontend, PR-C #476 — the last wires `default_global` function tools through `hierarchy.py:325` so `create_visualization` reaches every factory-built specialist by default). **AH-PRD-07 was superseded by AH-PRD-08** (Hide Strategy-Pipeline Specialists from the Workflows Picker) after a scoping pass confirmed the 8 strategy-pipeline specialists are account-creation-only and never re-invoked from chat — AH-PRD-08 hides them via the existing `visible_in_frontend=False` flag instead of rebuilding the construction path, same user-visible outcome with ~10× less code change. AH-PRD-08 shipped in R1. **AH-PRD-09 (Per-Turn Dispatch Agent)** is the runtime successor to AH-PRD-02 — replaces the deploy-time factory with a per-turn runtime resolver so admin agent edits take effect immediately (restores Sprint 6 Decision B's intent). AH-PRD-09 lands in R1 (Phases 0–3 + 5 — the `cloud_run`-only runtime resolver); its Phase 4 (Zapier hybrid MCP) defers to R2 alongside the Integrations component. AH-PRD-04 (Data Visualization) lands in Release 1 and adds chart-artifact output. **AH-PRD-05 (Multi-Step Workflow Orchestration — rewritten AH-97, 2026-06-01)** is gated on the ADK 2.0 Foundation PRD (AH-PRD-13) (not a release number) and is now the supervisor-orchestration spec: `LlmAgent(mode='chat')` coordinator + `mode='task'` specialist leaves + `ctx.run_node` fan-out — the prior `build_workflow_pipeline` / `execute_workflow` inner-Runner shape is retired. **AH-PRD-02b (Agent Identity Split — name + title)** shipped in R1 as a small follow-up to AH-PRD-02 (splits the storage-only `name` field into a user-editable `name` + role `title`). **AH-PRD-10 (Rate-Limiter Rework — user-keyed identity + Redis-backed shared state)** shipped in R1 as a cross-cutting platform-hardening fix — provisional component home, since the implementation lives under `api/src/kene_api/` rather than `app/adk/`. **AH-PRD-11 (Agent Reasoning & Inference Residency)** and **AH-PRD-12 (Observability Residency)** are data-residency launch blockers gated on EU Agent Engine GA (blocked by DM-PRD-09). Future per-platform specialist PRDs (Google Ads, Meta Ads, Mailchimp — see §2.6) land as AH-PRD-16+ (AH-PRD-10 through 15 are now taken — AH-PRD-13/14/15 are the ADK 2.0 migration initiative: Foundation / Contract-Preservation / AgentTool-Isolation), consuming the pattern established in AH-PRD-03 and automatically inheriting `create_visualization()` via the factory's default function-tool roster (see AH-PRD-04).
...
| 02b | Agent Identity Split (Name + Title) — no standalone PRD file (small follow-up to AH-PRD-02) | Core AI | AH-PRD-02 | — | **Shipped** (R1). Splits the storage-only `name` field on `agent_configs` into a user-editable `name` (human) + role `title`. |
...
| 06 | [Per-Agent Tool Mapping](./projects/AH-PRD-06-tool-mapping.md) | Core AI (backend + frontend) | AH-PRD-02 | SK-PRD-02 / SK-PRD-04 (shared form rows + ≤30-tool cap) | **Shipped** (R1). PR-A #472 (backend) + PR-B #473 (frontend) + PR-C #476 (`default_global` function-tool wiring). |
...
| 10 | [Rate-Limiter Rework — User-Keyed Identity](./projects/AH-PRD-10-rate-limiter-rework.md) | Core AI (provisional home — impl under `api/src/kene_api/`) | — | any AH-PRD work (low surface-area overlap) | **Shipped** (R1). Replaces the IP-keyed + in-memory `RateLimiter` with a user-keyed (Firebase UID) + Redis-backed `RedisRateLimiter`; fixes NAT'd-user DoS, cross-instance N×-threshold divergence, and `X-Forwarded-For` spoofing. Structurally fixes the DM-104 e2e flake. |
| 11 | [Agent Reasoning & Inference Residency](./projects/AH-PRD-11-agent-reasoning-inference-residency.md) | Core AI | DM-PRD-09, AH-PRD-09 | AH-PRD-12 | **Not started** — data-residency launch blocker (gated on EU Agent Engine GA): EU Agent Engine + regional Vertex model endpoint + `data_region`-threaded sandbox/session routing. |
| 12 | [Observability Residency](./projects/AH-PRD-12-observability-residency.md) | Core AI | DM-PRD-09, AH-PRD-11 | AH-PRD-11 | **Not started** — data-residency launch blocker: OTEL message-content capture OFF for EU deploys + EU-resident trace/log sink so EU prompt/response content never egresses to US observability. |
...
- **Strategy-agent specialists: hidden, not migrated (AH-PRD-06 → AH-PRD-08):** AH-PRD-07 originally planned to route `marketing_researcher` / `marketing_formatter` (tools hand-wired in `marketing_agents.py`, e.g. `google_search_agent`) onto the factory path, which would have required pre-backfilling their Firestore `tool_ids` / `mcp_servers` before the switch — otherwise the post-AH-PRD-06 empty-array semantics (`tool_ids: []`) would deploy them with **zero tools**. **AH-PRD-08 superseded that plan:** the 8 strategy-pipeline specialists are account-creation-only and never re-invoked from chat, so they are hidden from the picker via `visible_in_frontend=False` rather than migrated — no backfill needed. The touchpoint is retained as the rationale for the supersession.
...
2. **Day ~3 (AH-PRD-01 merged):** AH-PRD-02 kickoff. Backend and frontend can parallelize once the Phase 1 config loader lands — Phase 2 (backend-heavy: MCP + dispatch generation) and Phase 3 (frontend-heavy: UI + API) are largely independent once the Pydantic contract is published. AH-PRD-02b (name/title identity split) is a small follow-up once AH-PRD-02 lands.
3. **AH-PRD-09 (per-turn dispatch) — after AH-PRD-02.** Replaces AH-PRD-02's deploy-time factory with the runtime resolver (`specialist_runtime` + `transfer_to_agent`, Phase 2) and the `McpToolsetPool` (Phase 3); R1 ships Phases 0–3 + 5 (Phase 4 Zapier defers to R2). This is the runtime baseline the ADK 2.0 migration later migrates.
4. **AH-PRD-06 (per-agent tool mapping) [shipped] — after AH-PRD-02.** PR-A (#472) + PR-B (#473) + PR-C (#476, wires `default_global` function tools through `hierarchy.py:325`). **AH-PRD-08 (hide strategy-pipeline specialists) [shipped] — after AH-PRD-06**, superseding AH-PRD-07's construction-unification plan: hides the 8 strategy-pipeline specialists via the existing `visible_in_frontend=False` flag rather than rebuilding their construction path (see §5.3).
5. **AH-PRD-03 (GA specialist) — once AH-PRD-09 Phases 2 and 3 are in staging.** Gated by the per-turn dispatch runtime (`specialist_runtime` + `transfer_to_agent` surface, Phase 2 + AH-75) and the `McpToolsetPool` with the `kind="cloud_run"` `mcp_servers` schema field (Phase 3). Small project; mostly config + tests. Deprecation banner on `google_analytics_agent_v4.py`; full removal is a follow-up once no callers remain.
6. **AH-PRD-04 (data visualization):** kickoff once AH-PRD-03 is in staging. Phase 1 (Pydantic model → `create_visualization()` tool → `ChatResponse` extension → frontend renderer) can parallelize between backend and frontend once the `Artifact` model ships; Phase 2 (reviewer template + E2E against the GA specialist) requires Phase 1 in place. AH-PRD-06 PR-C is already in main by this point, so `create_visualization()` reaches every factory-built specialist automatically — AH-PRD-04 verifies the wiring rather than introducing it.
7. **ADK 2.0 migration chain — after AH-PRD-09 ships (the runtime baseline it migrates).** AH-PRD-13 (Foundation — unpin to 2.0 + breaking-change audit + deploy-tree decoupling, opening with a Phase-0 de-risking spike) → AH-PRD-14 (Contract Preservation — outer-stream / trace hardening, mostly on 1.34.1) → AH-PRD-15 (agent-as-tool **isolation** — `google_search`/`numerical_analyst` stay isolated `AgentTool`s with leaf-callback billing; owns the 2.0 prod-cutover go/no-go, cut over to prod 2026-06-07). Note: the planned `AgentTool → task-mode` migration was found unworkable for built-in-tool leaves (they 400 alongside the injected `FinishTaskTool`) and re-planned to AgentTool isolation — see [DESIGN-REVIEW-LOG Review 47](../../DESIGN-REVIEW-LOG.md#review-47--ah-prd-15-re-plan-agenttool-isolation-replaces-the-task-mode-migration-ah-121-prod-incident). The chain gates AH-PRD-05.
8. **AH-PRD-05 (multi-step orchestration):** kickoff after the ADK 2.0 migration chain (AH-PRD-13 → AH-PRD-14 → AH-PRD-15) lands and the `Artifact` convention is stable (AH-PRD-04 is a soft dependency — the supervisor threads per-task artifacts through the same path). Single Core AI track; the stories are largely sequential within a 3–5-day window. After this lands, the harness has a complete review-loop story across single-step and multi-step composition.
9. **AH-PRD-10 (rate-limiter rework) [shipped]:** cross-cutting platform hardening (user-keyed + Redis-backed limiter); no blockers, lands independently of the construction chain.
10. **Release 1 exit:** review loop, factory, per-turn dispatch, first specialist, per-agent tool mapping, data visualization, ADK 2.0 migration, and multi-step orchestration all working in staging. MER-E consuming Weave spans. Ready for downstream components (Skills, Project Tasks, KG-PRD-05) to pick up.
...
- **Supervisor-orchestration specialists carry `mode='task'`.** In the supervisor model (AH-PRD-05), leaf specialists are constructed with `LlmAgent(mode='task')` so the coordinator regains control after each task completes. The coordinator itself is `LlmAgent(mode='chat')`. Review `LoopAgent` leaves continue to wrap per-task specialist work as today. Task-mode nodes inside a static `Workflow` graph are NOT validated (GitHub `#3984` still OPEN) — use only `mode='task'` under a `mode='chat'` coordinator.

### Model resilience (429/transient-error retry)

Background and quantified rationale: [`docs/spike-vertex-429-long-running-tasks.md`](../../../spike-vertex-429-long-running-tasks.md) (2026-06-10 staging incident; Review 52).

- **Every Gemini call retries transient errors by default.** `ResilientGemini` (`agents/agent_factory/resilient_model.py`) is registered over ADK's `gemini-*` `LLMRegistry` patterns by `ensure_resilient_gemini_registered()`, called from `apply_model_location_env()` — which runs at deploy build time (`build_hierarchy`) and per chat turn (`attach_specialists_before_agent_callback`). String-model agents (root, specialists, reviewers, AgentTool leaves) get the genai backoff policy (5 attempts, 1–60s exponential + jitter, 408/429/5xx + connect/timeout) with no changes at construction sites.
- **Never construct a bare `Gemini(...)` instance.** A direct instance bypasses the registry and gets `retry_options=None`, which genai treats as **never retry** (`retry_args(None)` → `stop_after_attempt(1)`) — a single transient Vertex 429 then kills the whole turn. Use `ResilientGemini(...)` (see the compaction summarizer in `deploy_ken_e.py`).
- **The retry policy is uniform (deliberate, Review 52).** Google's guidance suggests fail-fast for interactive chat vs full backoff for background work; worst-case added latency before a terminal failure is ~15s, accepted for chat. Per-context (chat vs task/automation) tuning is a follow-up if latency telemetry warrants — the registry mechanism has no per-context hook today.
- **Streaming caveat:** genai's retry wraps request *initiation*; an error after first bytes on a token-streaming call is not retried. KEN-E's LLM calls are non-streaming today (`StreamingMode.NONE` default), so coverage is complete — revisit if token-level streaming is ever enabled.
- **`ReflectAndRetryToolPlugin` ≠ model retry.** The plugin (wired in `deploy_ken_e.py`) reflects on **tool execution** failures only; model-call `ClientError`s never reach it. Likewise `agent_retry.py` retries only `ConnectionError`/`TimeoutError`/`OSError`.
- **Vertex 429s are not quota.** Current Gemini models have no per-project quota (verified via the Cloud Quotas API); 429 = shared-pool contention on the serving endpoint, so quota-increase requests are not a remedy. Escape valves if backoff ever exhausts under real load: Priority PayGo header (+80%/token), Provisioned Throughput — see the spike's options matrix.
- **Model catalogue:** `shared/supported_models.py` is the single source (`SUPPORTED_MODELS`). The API layer rejects unknown models at the write boundary; the ADK config loader only **warns** (a raising validator would silently drop the agent from the hierarchy — the AH-85 lesson). Keep it synced with what Vertex actually serves: `gemini-2.0-flash` was discontinued upstream 2026-06-01; the 2.5 family retires no earlier than 2026-10-16.
- **Context-cache floor:** `ContextCacheConfig.min_tokens` must be ≥4096 — Gemini's server-side minimum. Below it, every `CreateCachedContent` call 400s and caching silently never engages.
...
- **Supervisor-orchestration span shape:** In multi-task supervisor-orchestrated turns, the trace hierarchy gains coordinator-level spans and per-task delegation spans (`task_delegation`, `fanout`). The MER-E trace contract for this shape is defined in [AH-PRD-05-trace-contract-diff.md](./projects/AH-PRD-05-trace-contract-diff.md) — MER-E must update extractors against the new fixture before any AH-PRD-05 implementation PR merges.
