...
2. For each ready task: `specialist_runtime.resolve_agent(config, mode='task')` returns a `mode='task'` specialist. The coordinator delegates via task mode's auto-injected `request_task_<name>` tool; the specialist runs to `complete_task`, at which point ADK returns control to the coordinator (the call-and-return primitive AH-99 validated). **Trap (AH-117 / AH-121):** ADK creates the dispatchable `_TaskAgentTool` only inside `LlmAgent.model_post_init`. Because specialists are resolved **per turn** and attached to the coordinator after construction, the implementation **must replicate the `_TaskAgentTool` injection by hand** (or construct coordinator + specialists together) — otherwise the coordinator's LLM never sees `request_task_<name>` and dispatch + its billing silently never fire. This must be proven with a **real-LLM (or stub-model-through-a-real-`Runner`) dispatch assertion**, not a construct-time / mock check — a construct-time gate masked exactly this defect in AH-117.
...
| **Per-turn task-mode dispatch silently no-ops (AH-117)** | A `mode='task'` specialist attached after construction has no `_TaskAgentTool` in `coordinator.tools` (ADK injects it only in `model_post_init`) → the coordinator's LLM never sees `request_task_<name>` → dispatch + billing never fire, and a construct-time/mock test still passes. Mitigation: replicate the `_TaskAgentTool` injection on the per-turn path (§5.2 step 2) and gate on a **real-LLM / stub-model-through-a-real-`Runner`** dispatch assertion. |
