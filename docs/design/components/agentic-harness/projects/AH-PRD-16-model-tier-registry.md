...
None of these validate that the model is currently served, and only the reviewer model has a runtime-editable, central knob. `model_routing.py` selects the Vertex *endpoint location* (`global` in dev; steady-state target `us` / `eu` **multi-region** in staging/prod — Review 50 — but currently **interim `global`** there too, Review 51) and is **orthogonal** to model selection — this PRD does not touch it, but the tier registry must account for its interaction. The endpoints do **not** carry identical catalogues: `gemini-3.5-flash` is served on `global` *and* `us`/`eu` multi-region, but **`gemini-3.1-pro-preview` is served on `global` only** — so "available on `global`" does **not** imply "served at `us`/`eu`" (the assumption Review 50 made, falsified by `gemini-3.1-pro-preview` — Review 51, and why staging/prod is interim-routed to `global`). The tier fallback list remains the backstop for this skew (a `global`-only model, or a dev-vs-prod difference).
...
| A model served only on some endpoints interacts with `model_routing.py` (e.g. `gemini-3.5-flash` 404s on single regions but is on multi-region; `gemini-3.1-pro-preview` is `global`-only, not even on multi-region — so staging/prod is interim-routed to `global`, Review 51). | Tier fallback list (`fastest` → `gemini-2.5-flash`) is the backstop for any residual endpoint/tier skew; document the interaction. |
