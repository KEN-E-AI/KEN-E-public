...
- **Production per-region model routing (R-03).** Extend the existing `apply_model_location_env()` call in the before-agent callback (`sub_agent_attacher.py:376`) to pass the per-turn account's `data_region`, so an EU account's genai client builds against the `eu` multi-region endpoint and a US account against `us`. This requires **restoring** `resolve_model_location`'s regional branches: they were `us`/`eu` after PR #964 (Review 50) but are **interim-collapsed to `global`** (Review 51) because `gemini-3.1-pro-preview` is `global`-only — so this PRD both (a) activates the `data_region` argument and (b) restores `us`/`eu`, gated on the model reaching the multi-region endpoints (the REVERT TRIGGER in `model_routing.py`). (The single-region `us-central1` / `europe-west1` strings still apply to `VERTEX_AI_LOCATION` — the engine/sandbox/session plane below — not to model serving.) The region is read from session state (`account_id` is already on `callback_context.state`, `sub_agent_attacher.py:422`) — resolve it via the foundation resolver, never via a blocking I/O call inside the callback (see §4.4).
...
- **The dev→`global` model slice (AH-86) + the staging/prod endpoint switches (Reviews 50 & 51).** The resolver (`model_routing.py`) shipped in PR #751 (dev→`global`); its regional branches were switched to `us` / `eu` multi-region in PR #964 (Review 50) and then **interim-collapsed to `global`** for the `global`-only `gemini-3.1-pro-preview` (Review 51). This PRD activates the production `data_region` branch (US-vs-EU per account) **and** restores the `us`/`eu` endpoint values — the latter gated on the model reaching the multi-region endpoints (the REVERT TRIGGER), not a free-standing choice.
...
`vertex_ai_location` and `location` are the *engine/session/sandbox* region (US→`us-central1`, EU→`europe-west1`). The *model-serving* location (`GOOGLE_CLOUD_LOCATION`) is **computed separately** by `model_routing.resolve_model_location` — its steady-state target is the **multi-region** endpoint (US→`us`, EU→`eu`; PR #964 / Review 50), currently **interim `global`** for the `global`-only `gemini-3.1-pro-preview` (Review 51). Either way model serving and the engine region **intentionally differ per cell** (they were equal before Review 50); model serving is therefore not a `CellConfig` field, and the two must never be conflated — the split the mechanism caveat is about.

### 4.2 Model-serving location resolution (R-03)

**Current resolver state (interim, Review 51):** `resolve_model_location` returns `"global"` for **every** environment because `gemini-3.1-pro-preview` is served on `global` only (not on the `us`/`eu` multi-region endpoints). The `data_region` argument is dormant. **This PRD performs two coupled changes:** it (a) adds the `data_region` call argument **and** (b) restores the `us`/`eu` per-account return values — but (b) is itself gated on `gemini-3.1-pro-preview` reaching the multi-region endpoints (the REVERT TRIGGER in `model_routing.py`; see data-residency-architecture.md §3.5). The target table below is the post-revert steady state.

| Environment / cell | `resolve_model_location(env, data_region)` → (target) | Applied via |
|---|---|---|
| `development` / `dev` | `"global"` | `apply_model_location_env()` (already shipped) |
| staging / prod, US account | `"us"` (US multi-region) — *interim `"global"`* | `apply_model_location_env(data_region="US")` |
| prod, EU account | `"eu"` (EU multi-region) — *interim `"global"`* | `apply_model_location_env(data_region="EU")` |
...
- `test_model_routing_data_region.py`: table over (`environment`, `data_region`) → expected location, incl. prod EU→`eu`, prod US→`us` (multi-region per Review 50 — the post-revert target; **note** the current resolver returns `global` for all non-dev per Review 51, so this table lands with the REVERT TRIGGER change), dev→`global` (any region) (AC-1); `.env`-inertness regression — set `GOOGLE_CLOUD_LOCATION` in env, assert `apply_model_location_env` overrides it (AC-2); assert no `VERTEX_AI_LOCATION` access in the model path (AC-3).
...
- **Q6 — EU region:** confirm `europe-west1` for the EU Agent Engine / sandbox / session plane (matches the GCS reference + DM-PRD-09), or a specific member-state region for sovereignty. *(Scope note: this is the `VERTEX_AI_LOCATION` plane only. EU **model serving** target is the `eu` multi-region endpoint — Review 50 — but is **interim-routed to `global`** because `gemini-3.1-pro-preview` is `global`-only; revert to `eu` before any EU account goes live, Review 51.)*
