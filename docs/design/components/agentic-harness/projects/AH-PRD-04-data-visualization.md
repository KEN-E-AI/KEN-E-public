...
**Blocks:** Any future specialist PRD that produces charts (Google Ads, Meta Ads, Mailchimp — see [README §2.6](../README.md#26-specialist-roadmap)) — they inherit `create_visualization()` through the factory's default function-tool roster
...
- **2.4-4 Frontend Vega-Lite rendering.** The renderer is **not yet on `main`** — the production frontend currently ships `recharts` and has no chat surface. The Figma export at `docs/figma-export/src/app/components/dashboard/ArtifactRenderer.tsx` (react-vega 8 / Vega-Lite 6.4.2) is the design contract. The production renderer is delivered by sibling components: [DB-PRD-03](../../dashboards/projects/DB-PRD-03-dashboard-details-and-canvas.md) creates `frontend/src/components/dashboards/widgets/VisualizationWidget.tsx` (react-vega-backed), and [CH-PRD-02](../../chat/projects/CH-PRD-02-chat-page-shell-and-sidebar.md) ports the chat-side renderer (`frontend/src/components/chat/ChatInterface.tsx` + chart block) from the same Figma source. Story 2.4-4 in this PRD wires `response.artifacts` into chat message state and reuses whichever renderer ships first; if neither is in place when 2.4-4 starts, this PRD owns the minimal port needed to render a Vega-Lite v6 spec inline. Malformed specs fall back to a collapsible raw-JSON block (`SpecFallback` in the Figma reference); text content always renders regardless of chart rendering.
...
| Frontend chart stack | Production frontend currently ships `recharts`; Vega-Lite is **not** yet on `main`. The renderer pipeline (react-vega 8 / Vega-Lite 6.4.2 / hover settings popover) is described in the Figma export under `docs/figma-export/src/app/components/dashboard/`. Production delivery is owned by [DB-PRD-03](../../dashboards/projects/DB-PRD-03-dashboard-details-and-canvas.md) (`VisualizationWidget`) and [CH-PRD-02](../../chat/projects/CH-PRD-02-chat-page-shell-and-sidebar.md) (chat surface). | `frontend/CLAUDE.md`, [`../data-visualization.md`](../data-visualization.md) §8 |
...
| Modify | `frontend/src/components/chat/ChatInterface.tsx` — thread `response.artifacts` into `Message.artifacts` and delegate to the renderer ported by [CH-PRD-02](../../chat/projects/CH-PRD-02-chat-page-shell-and-sidebar.md) / [DB-PRD-03](../../dashboards/projects/DB-PRD-03-dashboard-details-and-canvas.md) (figma-export source: `docs/figma-export/src/app/components/dashboard/ArtifactRenderer.tsx`). If neither sibling has shipped when this story starts, port the minimal renderer here. | 2.4-4 |
...
4. **Frontend chart rendering.** Vega-Lite chart artifacts render inline below chat-message text through the production renderer (delivered by CH-PRD-02 / DB-PRD-03, or ported here if neither sibling has shipped — see §2 story 2.4-4), with theme palette applied and the hover-reveal settings toolbar (chart type / color / data labels) available. Multiple artifacts render in list order. The text content always renders regardless of chart rendering outcome.
...
- Frontend renderer (Figma export, design contract): `docs/figma-export/src/app/components/dashboard/ArtifactRenderer.tsx`, `docs/figma-export/src/app/components/dashboard/TileSettingsPopover.tsx`, `docs/figma-export/src/app/components/dashboard/artifactTypes.ts`. Production-tree equivalents are delivered by [DB-PRD-03](../../dashboards/projects/DB-PRD-03-dashboard-details-and-canvas.md) (`frontend/src/components/dashboards/widgets/VisualizationWidget.tsx`) and [CH-PRD-02](../../chat/projects/CH-PRD-02-chat-page-shell-and-sidebar.md) (chat surface).
