...
- **Unifying the `strategy_agent` construction path with the factory** — agents built via `app/adk/agents/strategy_agent/config_loader.py:create_agent_from_firestore_config` (the 8 strategy-pipeline specialists: `business_researcher` / `business_formatter`, `competitive_researcher` / `competitive_formatter`, `marketing_researcher` / `marketing_formatter`, `brand_researcher` / `brand_formatter`) read the same Firestore documents but go through a separate code path that strips `tool_ids` and `mcp_servers` before construction. Originally tracked as AH-PRD-07; resolved differently by **[AH-PRD-08](./AH-PRD-08-hide-strategy-pipeline-specialists.md)** — see the **Resolution** subsection below.

### Resolution for strategy-pipeline specialists (AH-PRD-08)

The 8 strategy-pipeline specialists are account-creation-only (invoked exactly once via `create_strategy_docs_supervisor.py`, never via the runtime chatbot — see [AH-PRD-08 §1](./AH-PRD-08-hide-strategy-pipeline-specialists.md#1-context) for the call-site audit). Even after a hypothetical migration onto the factory path, picker-driven tool changes would never affect a *next* invocation because there is no next invocation. Rather than rework the construction path, **[AH-PRD-08](./AH-PRD-08-hide-strategy-pipeline-specialists.md)** hides these 8 agents from the `/workflows/agents` picker (via the existing `visible_in_frontend` flag) so the picker contract is not made for them in the first place. The four *formatters* are already hidden today via `AUDIT_FIELDS_FORMATTER`; AH-PRD-08 splits the researcher seed profile so the four *researchers* are also hidden, plus a one-off Firestore backfill to bring deployed environments in line.

**Surfaces affected by the original gap:** any specialist created in `app/adk/agents/strategy_agent/` (the 8 listed above, plus any future specialist added through the same loader). Specialists built via `app/adk/agents/agent_factory/hierarchy.py:build_hierarchy` (the AH-PRD-02 path) were never affected — `tool_ids` works correctly there.
