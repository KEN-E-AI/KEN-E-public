...
| `agent_configs/{id}.skill_ids` | `SkillToolset(skills=...)` (via `_build_skill_toolset(account_id, skill_ids)`) | Default `[]`. For each ID, calls `skill_loader.load_skill(account_id, skill_id)` (SK-PRD-01) and hydrates a `SkillToolset` attached to `LlmAgent.tools`. Missing/archived skills are logged and skipped; the agent still builds. See [SK-PRD-02 §4 data contract](../../skills/projects/SK-PRD-02-agent-integration.md#4-data-contract). |
| `agent_configs/{id}.sandbox_code_executor_enabled` | `LlmAgent.code_executor = AgentEngineSandboxCodeExecutor(...)` (via `_build_code_executor` → `SandboxPool.get_or_create`) | Default `False`. When `True`, attaches a pooled `AgentEngineSandboxCodeExecutor` as the agent's `code_executor`; the pool is keyed by `(account_id, config_id)` with LRU + idle-TTL eviction so AH-PRD-09's per-turn `LlmAgent` rebuild reuses the underlying sandbox process. Distinct from `code_execution_enabled` (Gemini built-in). See [SK-PRD-02 §4.6 SandboxPool](../../skills/projects/SK-PRD-02-agent-integration.md#46-sandboxpool). |
