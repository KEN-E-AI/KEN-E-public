...
| `agent_configs/{id}.sandbox_code_executor_enabled` *(forward-compat pass-through)* | Pass-through — [`SandboxPool`](../../skills/projects/SK-PRD-02-agent-integration.md#46-sandboxpool) library landed in SK-23; `_build_code_executor` wiring lands in SK-26 | Default `False`. Distinct from `code_execution_enabled` (Gemini built-in). When SK-26 ships, this will attach a pooled `AgentEngineSandboxCodeExecutor` keyed by `(account_id, config_id)`. |
