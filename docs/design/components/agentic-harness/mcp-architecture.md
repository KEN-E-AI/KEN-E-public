...
**6. Post-construction task-mode sub-agent wiring requires the `_TaskAgentTool` injection done by hand.** ADK creates the `_TaskAgentTool` (the marker that lets a chat-mode parent dispatch `request_task_<name>`) ONLY inside `LlmAgent.model_post_init`. Because the production per-turn path resolves and attaches specialists **after** the coordinator/root is already constructed, `model_post_init` runs without them — the tool is never added to the parent's `tools` list. Without it, the coordinator's LLM never sees `request_task_<name>` and delegation (plus its billing) silently no-ops even though all construct-time and mocked tests pass green. This is the exact defect that appeared in AH-117 (the original AH-PRD-15 task-mode migration).
