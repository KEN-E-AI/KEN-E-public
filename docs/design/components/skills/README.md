...
Beyond user-authored skills, KEN-E ships predefined skills attached to **specialists** (not the root). Under [AH-PRD-09](../agentic-harness/projects/AH-PRD-09-per-turn-dispatch.md) Phase 2 the deployed root carries only `delegate_to_specialist` — it has no `SkillToolset` and no `list_skills` tool, so `skill_ids` on the root config has no effect. System skills therefore attach to specialist `agent_configs/*` documents. These are stored separately from per-account skills:
...
v1 ships exactly one placeholder system skill (`example-skill`) attached to `agent_configs/google_analytics_specialist` (the R1 specialist, shipped by [AH-PRD-03](../agentic-harness/projects/AH-PRD-03-google-analytics-specialist.md)). Real predefined-skill content (e.g., `kene-ga-attribution-checklist`) is the work of follow-up content PRDs; once SK-PRD-05 lands, those are content-only changes (one SKILL.md + one config update). Broader rollout — where every runtime-resolved specialist receives one or more system skills automatically — is a follow-up PRD that introduces a `default_global: true` skills mechanism analogous to [AH-PRD-06](../agentic-harness/projects/AH-PRD-06-tool-mapping.md) PR-C's `default_global` function-tool injection.
