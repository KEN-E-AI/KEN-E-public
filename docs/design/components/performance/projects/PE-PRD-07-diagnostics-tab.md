...
- **DP-PRD-01 (Data Pipeline foundation):** owns `DataPipelineRun` + `accounts/{account_id}/data_pipeline_runs/*`. A new query — "for each KPI's `source_job_id`, return the latest-successful-run timestamp, the latest-week-ingested, a gap count, and a 53-week coverage percentage" — is required for the ingestion-health section. The query itself is a DP-PRD-01 deliverable; this PRD consumes it via the bundle composer.
...
3. For each mapped KPI, read the latest `Baseline` doc (SE-PRD-03) and the matching IRF-convergence snapshot (SE-PRD-04). Assemble `PerKPIModelHealth`.
4. For each mapped KPI, derive `KPIIngestionStatus` from DP-PRD-01's ingestion-status query (one Firestore aggregation across `accounts/{account_id}/data_pipeline_runs` filtered by `job_id == EffectivenessKPI.source_job_id` + `kpi_time_series` reads for the gap count).
...
| `retrain_needed` + reason field may not yet exist on SE-PRD-03's `Baseline` model | Confirm at kickoff. If not, extend `Baseline` in SE-PRD-03 (small additive change) rather than inferring in the bundle composer; inference would duplicate SAR-E's domain logic in Performance. |
...
2. **Is `retrain_needed` already emitted by SAR-E, or is this PRD assuming a field that needs to be added?** If SE-PRD-03 does not yet expose it, extend that PRD's scope rather than inferring here. Assume extension is in-scope for kickoff.
