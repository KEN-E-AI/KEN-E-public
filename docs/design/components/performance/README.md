...
Performance is KEN-E's **marketing-measurement page** — a six-tab surface (`Analysis`, **`Dashboards`**, `Simulations`, `Targets`, `Diagnostics`, `Configuration`, in Figma order) at `/performance` that shows how the business is expanding its conversion funnel, what its 12-week forecast looks like, which per-KPI Targets are in effect, how healthy the forecasting model + ingestion pipeline are, and exposes the user's pinned-artifact **Dashboards** alongside the analytical tabs. The component also owns the four-step **Setup Wizard** at `/performance/setup` that turns an opted-out account into a fully configured SAR-E consumer. The Dashboards tab content itself is owned by the [Dashboards component](../dashboards/README.md) (DB-PRD-02 list / DB-PRD-03 details); PE-PRD-01 reserves the slot, route (`/performance/dashboards`), placeholder, and feature flag (`performance_dashboards_tab`).
...
Performance is **opt-in per account.** SAR-E is disabled at account creation. Pre-wizard, only **Dashboards + Configuration** are visible in the nav (the four SAR-E-backed tabs are hidden by `ForecastingEnabledGate`); Configuration renders an empty-state CTA pointing at the wizard, and Dashboards renders DB-PRD-02's list (or its empty state) without gating; `/performance` redirects to `/performance/configuration`. Post-wizard, SAR-E's `/config/status` flips to `enabled=true`, all six tabs appear, and `/performance` redirects to `/performance/analysis`. A developer reading only this section should understand: this component owns the `/performance` page shell, the five SAR-E-backed tab UIs (Analysis / Simulations / Targets / Diagnostics / Configuration), the setup wizard, five composite BFF endpoints, a `performance_wizard_draft` Firestore doc, and the Figma-export-to-production terminology rename (`Goal` → `Target`). The **Dashboards tab UI is filled in by the Dashboards component**, not here. It does **not** own forecasting, target persistence, analytical queries, KPI ingestion, or the OAuth connection UI.
...
| Path | Purpose |
|------|---------|
| `frontend/src/pages/Performance/PerformancePage.tsx` | Page shell, six-tab router, default-route redirect logic. PE-PRD-01. |
...
| `frontend/src/lib/featureFlags/registry.ts` | Seven flag keys registered here: `performance_{analysis,dashboards,simulations,targets,diagnostics,configuration}_tab` + `performance_setup_wizard`. PE-PRD-01. |
...
| **[UI](../ui/README.md)** | UI-PRD-01 (Design System Foundation) supplies Soft Maximalism tokens, shadcn primitives, the global shell, sidebar slot, and the `bannerSlot` outlet on `LayoutC`. Performance registers as a new top-level nav entry. **UI-PRD-07 is retired and subsumed by PE-PRD-01** — this component's page shell replaces the legacy `Presentation.tsx` entirely; no coordination with the (retired) UI-PRD-07 remains. | [`../ui/README.md`](../ui/README.md), [`../ui/projects/UI-PRD-01-design-system-foundation.md`](../ui/projects/UI-PRD-01-design-system-foundation.md) |
| **[Dashboards](../dashboards/README.md)** | Dashboards owns the **2nd tab** (`/performance/dashboards`) of this component's page. PE-PRD-01 reserves the slot + placeholder; DB-PRD-02 swaps in the list view; DB-PRD-03 owns the details page at `/performance/dashboards/{plan_id}`. The Dashboards tab is **not gated by `ForecastingEnabledGate`** — visible regardless of SAR-E enablement. | [`../dashboards/README.md`](../dashboards/README.md), [`../dashboards/projects/DB-PRD-02-dashboards-tab-and-list.md`](../dashboards/projects/DB-PRD-02-dashboards-tab-and-list.md) |
| **[Feature Flags](../feature-flags/README.md)** | FF-PRD-03 publishes `FeatureFlagsProvider` + `useFeatureFlag(key)`. Seven flag keys registered by PE-PRD-01: `performance_{analysis,dashboards,simulations,targets,diagnostics,configuration}_tab` + `performance_setup_wizard`. Each tab is visibility-gated by its flag; the four SAR-E-backed tabs (Analysis / Simulations / Targets / Diagnostics) are additionally gated by the enablement check. The Dashboards + Configuration tabs are visible regardless of SAR-E enablement (subject to their own flags). The wizard flag is an independent kill-switch path. | [`../feature-flags/README.md`](../feature-flags/README.md), [`../feature-flags/projects/FF-PRD-03-frontend-sdk-and-e2e.md`](../feature-flags/projects/FF-PRD-03-frontend-sdk-and-e2e.md) |
...
|----------|----------|--------------|
| Figma: [KEN-E UI V2 — Soft Maximalism](https://www.figma.com/make/fhkgWZyTHdKtvDNRoQrcMT/KEN-E-UI-V2---Soft-Maximalism) | Performance page (all six tabs + wizard) | Starting any PE-PRD that ships a UI. Rebuild components against Soft Maximalism tokens — do not literally copy the Figma export. |
...
**Total effort:** ~22–27 engineer-days. After PE-PRD-01 lands, the five SAR-E-backed tabs + wizard are maximally parallel (the Dashboards tab is filled in by DB-PRD-02 / DB-PRD-03 in the Dashboards component, also after PE-PRD-01); the critical path runs PE-PRD-01 → (PE-PRD-03 or PE-PRD-05) → PE-PRD-08.
...
- **Accessibility audit (PE-PRD-08):** automated axe-core pass across all six tabs + wizard; manual keyboard-navigation pass.
