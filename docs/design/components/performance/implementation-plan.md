...
The Performance component is KEN-E's marketing-measurement dashboard — a five-tab page (`Analysis`, `Simulations`, `Targets`, `Diagnostics`, `Configuration`) that surfaces how the business is expanding its conversion funnel, what its calendar of activities is predicted to do over the next 12 weeks, the current KPI target values with actuals-to-date, and forecast-model health. Performance owns all five tabs; SAR-E provides the data for Targets and Diagnostics.
...
**Performance is opt-in.** SAR-E is disabled at account creation. The Configuration tab shows an empty-state CTA linking to the setup wizard, and the following tabs are hidden from the nav until the wizard completes: Analysis, Simulations, Targets, Diagnostics. Pre-wizard, `/performance` redirects to `/performance/configuration`; post-wizard, it redirects to `/performance/analysis`. Once forecasting is enabled, all five tabs render live data against SAR-E's weekly KPI series, 12-week baseline, and per-KPI targets.
...
- The `/performance` page shell with tab routing (five tabs).
...
**Delivers:** `/performance` page registered in the global shell with five tabs (Analysis, Simulations, Targets, Diagnostics, Configuration); `PerformanceDateRangeContext` provider (week-indexed period + comparison-mode); `ForecastingEnabledGate` that renders the Configuration empty-state CTA and hides Analysis / Simulations / Targets / Diagnostics from the tab nav until SAR-E `/config/status` flips to `enabled=true`; default-route logic (`/performance` → `/performance/configuration` pre-wizard, → `/performance/analysis` post-wizard); dedicated `/performance/setup` route reserved for the wizard; `FunnelObjective` / `EffectivenessKPIId` / `ComparisonMode` / `CostDimension` / `WizardStep` branded types in `frontend/src/types/performance.ts`; terminology-rename scaffolding (`useTargets` hook wrapping SAR-E `/targets`); feature-flag wiring for all five tab flags + wizard flag. No live data yet.
...
- New account → lands on `/performance` (auto-routed to `/performance/configuration`) → sees only the Configuration tab with empty-state CTA (other four tabs hidden) → clicks wizard → connects Google (via Integrations) → completes wizard → forecasting enables → all five tabs appear → redirect to Analysis → 53-week trendlines (low-confidence baseline) + cost rollups render.
