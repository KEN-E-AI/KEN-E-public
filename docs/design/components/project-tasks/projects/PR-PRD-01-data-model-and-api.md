...
Auth: standard `check_strategy_access`-equivalent dependency, scoped per account. Role enforcement on status-changing transitions is delegated to `require_role` + `assert_transition_allowed` from [DM-PRD-07](../../data-management/projects/DM-PRD-07-approval-workflow-and-audit.md); this PRD ships the raw transition endpoints and calls the gate. Responses use the same `200 / 201 / 403 / 404 / 409 / 422` patterns as the strategy router (with `409` reserved for disallowed transitions).
