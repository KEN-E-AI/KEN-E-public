...
| `deployment/firestore.rules` | **New** — security rules enforce `resource.data.user_id == request.auth.uid` on `chat_sessions/*`, `request.auth.uid == userId` on `chat_categories/*`, artifact subcollection reads gated by parent-session ownership (client-side writes disabled). Deployed by `deployment/terraform/firestore_rules.tf`. (CH-PRD-01) |
