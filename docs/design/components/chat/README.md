...
| `app/adk/token_accounting.py` | `extract_billable_tokens(event) → BillableTokenCounts`. **Billing-owned** (per BL-PRD-02); Chat consumes. (CH-PRD-01 lands the helper under Billing's namespace if BL-PRD-02 hasn't shipped.) |
...
| `extract_billable_tokens(event)` | `app/adk/token_accounting.py` | **Billing-owned** per BL-PRD-02. Shared helper: input + output + reasoning; cached-input excluded. Chat consumes. |
...
| **[Data Management — DM-PRD-07](../data-management/projects/DM-PRD-07-approval-workflow-audit.md)** | **Soft.** `write_audit` for chat-session lifecycle events. Optional at v1 — plain structured logging is acceptable if DM-PRD-07 hasn't shipped. | [`../data-management/README.md`](../data-management/README.md) audit section |
...
| **[Feature Flags — FF-PRD-01](../feature-flags/projects/FF-PRD-01-data-model-evaluation-api-backend-sdk.md), [FF-PRD-03](../feature-flags/projects/FF-PRD-03-frontend-sdk-e2e.md)** | **Hard prerequisite.** Three flags: `chat_v2_enabled` (master kill switch — reverts UI to pre-component state), `chat_status_detail_enabled` (gates the status view separately), `chat_categories_enabled` (gates CH-PRD-03 surface). **No `chat_manual_compaction_enabled`** (Compact-now out of v1). **No placeholder-cards flag** (Permissions / Tools UIs out of v1). | [`../feature-flags/README.md`](../feature-flags/README.md) |
...
| **ADK `VertexAiSessionService`** (existing) | Session CRUD, event stream, `session.state` read/write, `list_sessions(app_name, user_id)`. `app_name="ken_e_chatbot"`. See [`app/adk/session/recovery.py`](../../../app/adk/session/recovery.py). | Google ADK Python v1.16+ |
...
| Existing `/api/v1/chat/*` surface (9 endpoints) | Base surface that CH-PRD-01..05 extend. | [`api/src/kene_api/routers/chat.py`](../../../api/src/kene_api/routers/chat.py) |
| Redis metadata cache (existing, 24h TTL) | Preserved as a speed-up; the side-table is authoritative. | [`api/src/kene_api/cache.py`](../../../api/src/kene_api/cache.py) |
