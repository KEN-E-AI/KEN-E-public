...
- **Audit integration via DM-PRD-07's generalized `write_audit`** — Billing lifecycle events persisted under `organizations/{org_id}/billing_audit/{audit_id}` by calling `write_audit(parent_kind="organization", parent_id=org_id, audit_subcollection="billing_audit", resource_type=<one of "billing_profile"|"subscription"|"checkout_session"|"invoice"|"manual_override">, action=<registered Billing action>, ...)`. Billing is the only org-scoped audit consumer in v1; the `(parent_kind, audit_subcollection, resource_type, actions)` tuples are pinned in DM-PRD-07's audit registry. A thin `write_billing_audit(...)` wrapper at `api/src/kene_api/billing/audit.py` packages the billing-specific metadata (subscription_id, tier_stop_index, etc.) and forwards to `write_audit`.
...
- Org-admin authorization on state-changing endpoints — BL-PRD-05 (no state-changing endpoints exist yet).
...
| **[DM-PRD-07](../../data-management/projects/DM-PRD-07-approval-workflow-and-audit.md)** | Generalized `AuditEntry` schema + `write_audit(parent_kind, parent_id, audit_subcollection, resource_type, action, ...)` helper + per-component registry. Billing registers `(organization, billing_audit, {billing_profile, subscription, checkout_session, invoice, manual_override}, {profile_created, checkout_started, subscription_created, subscription_updated, subscription_canceled, invoice_paid, invoice_failed, manual_override_applied, sales_handoff})` and uses the shared writer via `write_billing_audit`. Also publishes `OrgRole = admin \| member` and `require_role(min_role, scope="org")` — Billing's permission middleware wraps `require_role(OrgRole.ADMIN, scope="org")` directly (see BL-PRD-05). | `../../data-management/README.md` §2.3 audit row, §4.1 role enums |
