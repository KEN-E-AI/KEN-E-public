...
**Blocked by:** SE-PRD-01 (`EffectivenessKPI`, `SarEConfig`, `/config/setup` transaction + automation seeder entry point); DP-PRD-01 (`DataPipelineJob`, `DataPipelineRun`, `PipelineOutput` contract); DP-PRD-02 (the four SAR-E-specific GA daily jobs — `ga.unbranded_search_daily`, `ga.branded_search_daily`, `ga.pdp_views_daily`, `ga.first_purchases_daily`); DP-PRD-03 (`TaskOrchestrator` `data_pipeline` branch — the ingestion automation's extraction tasks run through this); AH-PRD-02 (agent factory builds the `sar_e_ingestion` glue agent from `agent_configs/sar_e_ingestion`); A-PRD-01 (`ProjectPlan` automation fields + `PlanRun` model — the real ingestion plan replaces SE-PRD-01's stub here); A-PRD-02 (recurring scheduler + `{inputs.*}` substitution / time macros — `0 7 * * 1 UTC` cron fires the weekly plan); A-PRD-03 (`TaskArtifact` system — Data Pipeline extract tasks publish artifacts the ingest task consumes); A-PRD-04 (`is_test` honoring on `PlanRun` — backfill plans must respect test-mode propagation)
