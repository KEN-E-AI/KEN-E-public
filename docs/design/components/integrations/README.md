...
| `/api/v1/internal/integrations/credentials/{account_id}/{platform_id}` | GET | IN-PRD-01 (refresh by IN-PRD-02; `platform_metadata` by IN-PRD-04) | OIDC. Decrypted `{connection_id, access_token, expires_at, external_account_id, platform_metadata}`. Transparently refreshes. |
...
1. `GET /api/v1/internal/integrations/credentials/{account_id}/{platform_id}` with the service's OIDC token. Response: `{connection_id, access_token, expires_at, external_account_id, platform_metadata}`.
2. Use the returned `access_token` against the platform. For platforms that ride extra routing data (Mailchimp's `dc`), read it from `platform_metadata`. **Do not cache the access_token across tasks** — re-read at the start of each unit of work so refresh events propagate.
3. On platform 401 (or equivalent): `POST /api/v1/internal/integrations/connections/{connection_id}/mark-expired` with `{account_id, reason}`. The `connection_id` was returned in step 1 — no separate lookup.
