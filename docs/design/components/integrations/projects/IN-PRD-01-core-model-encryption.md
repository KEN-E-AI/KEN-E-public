...
- **Audit integration via DM-PRD-07's generalized `write_audit`** — lifecycle events persisted under `accounts/{account_id}/integrations_audit/{audit_id}` by calling `write_audit(parent_kind="account", parent_id=account_id, audit_subcollection="integrations_audit", resource_type="platform_connection", action=<one of "connected"|"refreshed"|"revoked"|"reauth_requested"|"used"|"error"|"tested">, ...)`. The `(parent_kind, audit_subcollection, resource_type, actions)` tuple is registered in DM-PRD-07's audit registry; writes outside the registered set are rejected at runtime. A thin `write_connection_audit(...)` wrapper at `api/src/kene_api/integrations/audit.py` packages the platform-specific metadata and forwards to `write_audit`.
...
| **[DM-PRD-07](../../data-management/projects/DM-PRD-07-approval-workflow-and-audit.md)** | Generalized `AuditEntry` schema + `write_audit(parent_kind, parent_id, audit_subcollection, resource_type, action, ...)` helper + per-component registry. Integrations registers `(account, integrations_audit, platform_connection, {connected, refreshed, revoked, reauth_requested, used, error, tested})` and uses the shared writer via the thin `write_connection_audit` wrapper. | `../../data-management/README.md` §2.3 audit row |
