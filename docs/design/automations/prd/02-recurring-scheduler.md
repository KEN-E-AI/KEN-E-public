...
This PRD reuses the Cloud Scheduler infrastructure delivered by [Calendar PRD-6](../../prd/06-time-based-scheduler.md) but adds a sibling endpoint, since per-task firing and per-automation firing have different queries and different downstream paths.
...
- **Inputs template substitution in the orchestrator's prompt builder**: when a run carries `inputs`, agent-task prompts get `{inputs.key}` placeholders replaced at dispatch time (owned here; consumed by KG-PRD-4 and any other system-triggered automation)
...
- `triggered_by="system"` — used by platform services (e.g. KG-PRD-4's idle-session sweeper) that kick off runs imperatively. The endpoint MAY allow `triggered_by_user_id=null` in this case; callers running under a service account identity pass that through.
...
The internal endpoint MUST NOT be exposed to user authentication. The manual-trigger endpoint uses the same access-control dependency as A-PRD-1.
...
- Parent plan: [`../automations-implementation-plan.md`](../automations-implementation-plan.md) §2 (Architectural overview)
- Foundation: [A-PRD-1](./01-data-model-and-api.md)
- Reused infra: [Calendar PRD-6](../../prd/06-time-based-scheduler.md), [Calendar PRD-4](../../prd/04-event-driven-orchestrator.md)
