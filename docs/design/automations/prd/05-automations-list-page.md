...
`is_system: bool` on `ProjectPlan` (Calendar PRD-1) marks a template as platform-owned rather than user-authored. Known use case: the session-end automation seeded by KG-PRD-4.
...
- Parent plan: [`../automations-implementation-plan.md`](../automations-implementation-plan.md) §5 (Phase 5)
