...
- **Read-only mode when `is_system=true`** — used for platform-owned templates (e.g. KG-PRD-4's session-end automation). See §is_system read-only mode.
...
| Right-side panel — status actions (**Mark Complete**, **Revision Requested**) | Visible on HITL tasks | **Visible** — this is the core interaction: reviewers approve or request revisions on halted runs (e.g. KG-PRD-4 proposals) without editing the template |
...
- Parent plan: [`../automations-implementation-plan.md`](../automations-implementation-plan.md) §5 (Phase 6)
