...
- **A-PRD-1:** `PlanRun` model + `plan_runs_{account_id}` collection (sub-collection lives under each run)
...
3. The file metadata appears in Firestore `plan_runs_{account_id}/{run_id}/artifacts/`
...
- Parent plan: [`../automations-implementation-plan.md`](../automations-implementation-plan.md) §2 (Architectural pillar 3)
- Foundation: [A-PRD-1](./01-data-model-and-api.md), [Calendar PRD-4](../../prd/04-event-driven-orchestrator.md)
