...
| **[PLANNED] Per-turn dispatch agent** — Successor to the deploy-time factory above. Deployed root becomes a thin dispatcher with one tool (`delegate_to_specialist`); specialists are resolved per turn from Firestore via `specialist_runtime` (TTL + content-hash cache, per-key striped locking). Admin agent edits (instruction / model / temperature / max_output_tokens / tools / new specialists) propagate to the next chat turn without redeploy — restores Sprint 6 Decision B's intent. Hybrid MCP via `McpServerKind` enum (`cloud_run` + `zapier`) routes long-tail integrations behind a single shared Zapier MCP connection per account while flagship integrations stay on owned Cloud Run servers. Tracked as [AH-PRD-09](design/components/agentic-harness/projects/AH-PRD-09-per-turn-dispatch.md) (Release 1; Phase 4 Zapier deferred to R2 alongside Integrations); see [`docs/design/per-turn-dispatch-rfc.md`](design/per-turn-dispatch-rfc.md) for the full design. |
...
- [**[PLANNED] Per-turn dispatch agent (R1, supersedes AH-PRD-02 in the runtime path)**](design/components/agentic-harness/projects/AH-PRD-09-per-turn-dispatch.md) — AH-PRD-09: deployed root becomes a single-dispatch agent (`delegate_to_specialist`); specialists resolved per turn from Firestore via `specialist_runtime`; admin edits propagate without redeploy; hybrid MCP kinds via `McpServerKind` (`cloud_run` + `zapier`) with `McpToolsetPool` reusing connections across runtime rebuilds. Full design in [`docs/design/per-turn-dispatch-rfc.md`](design/per-turn-dispatch-rfc.md). **The deploy-time wording elsewhere in this section continues to describe the current shipped state**; collapses to the runtime model when AH-PRD-09 phases ship.
