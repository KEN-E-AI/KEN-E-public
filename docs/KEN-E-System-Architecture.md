...
- [**Per-turn dispatch agent (R1, supersedes AH-PRD-02 in the runtime path)**](design/components/agentic-harness/projects/AH-PRD-09-per-turn-dispatch.md) — AH-PRD-09 + [AH-75](https://linear.app/ken-e/issue/AH-75): deployed root carries no specialist-dispatch tool (`tools=[]`); specialists are resolved per turn from Firestore via `specialist_runtime` and attached to `root.sub_agents` per turn by `sub_agent_attacher`; the root LLM dispatches via ADK's native `transfer_to_agent`. Admin edits propagate without redeploy. `McpToolsetPool` reuses MCP connections across runtime rebuilds (LRU + idle-TTL eviction; `aclose()`-on-eviction); `mcp_servers/{server_id}` carries a `kind` field (`cloud_run`; `zapier` kind deferred). **[PLANNED] Phase 4 (R2)** Zapier first-class integration is deferred alongside Integrations. Full design in [`docs/design/per-turn-dispatch-rfc.md`](design/per-turn-dispatch-rfc.md); dispatch-surface decision in [DESIGN-REVIEW-LOG Review 35](design/DESIGN-REVIEW-LOG.md).
...
- **MCP connections are reused across per-turn specialist rebuilds via `McpToolsetPool` (AH-PRD-09 Phase 3).** `mcp_servers/{server_id}` carries a `kind` field (`cloud_run`; open enum). The process-wide `McpToolsetPool` keys entries by `(server_id, account_id, auth_hash)`, evicts via LRU + idle-TTL, and calls `aclose()` on eviction. Tool rosters resolve at runtime via `specialist_runtime` — no longer fixed at deploy time. Flagship integrations stay on owned Cloud Run servers. **[PLANNED] Phase 4 (R2):** Long-tail integrations will route through one Zapier MCP connection per account (`accounts/{account_id}/integrations/zapier`) once the Integrations component ships. See [`docs/design/per-turn-dispatch-rfc.md`](design/per-turn-dispatch-rfc.md) §4.2.1, §4.6, §4.8.
...
| **Model invocation (every Gemini call)** | `ResilientGemini` registered over the `gemini-*` patterns in ADK's `LLMRegistry` (from `apply_model_location_env()`, which runs at deploy build and per chat turn) defaults `retry_options` to the genai backoff policy: 5 attempts, 1–60s exponential + jitter, on 408/429/5xx + connect/timeout. Without it, genai treats `retry_options=None` as **never retry** — a single transient Vertex 429 used to kill an entire multi-step turn (2026-06-10 staging incident; see [`docs/spike-vertex-429-long-running-tasks.md`](spike-vertex-429-long-running-tasks.md)). Covers root, specialists, reviewers, and AgentTool leaves uniformly. Caveat: retry wraps request *initiation*; a failure mid-stream on a token-streaming call is not retried (KEN-E's LLM calls are non-streaming today — `StreamingMode.NONE`). | `agents/agent_factory/resilient_model.py` |
| **Agent invocation** | Exponential backoff + jitter via `@retry_with_exponential_backoff()`. Retriable: `ConnectionError`, `TimeoutError`, `ValidationError`. 3 attempts, 1-30s delay. Does **not** cover model-call `ClientError`s (429 etc.) — those are the Model-invocation row's job. | `agent_retry.py` |
...
#### 11.3.3 Vertex Model-Serving Capacity

Distinct from platform-API quotas above: a `429 RESOURCE_EXHAUSTED` from Vertex model serving is **not a per-project quota** for current Gemini models — they carry no per-project RPM/TPM quota at all (verified via the Cloud Quotas API, 2026-06-10). Capacity is governed by org-spend Usage Tiers on the `global` endpoint (the largest pool and Google's recommended 429 remedy #1); a 429 means momentary shared-pool contention, so quota-increase requests are not a remedy. The remedies, in order: client-side backoff retry (implemented — §11.1.1 Model-invocation row), traffic smoothing (second-level spikes throttle even at trivial per-minute volume), and paid tiers (Priority PayGo +80%/token via one header; Provisioned Throughput from $1,200/GSU-week). Full capacity landscape + options matrix: [`docs/spike-vertex-429-long-running-tasks.md`](spike-vertex-429-long-running-tasks.md).
...
| **Vertex shared-capacity 429 mid-turn (long multi-step tasks)** | Medium | High | `ResilientGemini` backoff retry on every model call (§11.1.1). Survival math: a 2-h unattended task ≈0.01% without retry vs >99.99% with it. Remaining gaps: checkpoint/resume on retry exhaustion; Priority PayGo / PT escape valves (spike doc options O-3/O-5/O-7). |
...
| **Token cost increase from review loops** | High | Medium | Reviewer model defaults to `gemini-2.5-pro` (`DEFAULT_REVIEWER_MODEL`, per-specialist override via `reviewer_model` — AH-92/93; `gemini-2.0-flash` was discontinued upstream 2026-06-01); criteria generation ~200 tokens; monitored via Weave. |
...
|---------|-------|--------------------------------|
| **1** | Foundation | Data Management Shape B migration (DM-PRDs 00–06); Agentic Harness — review loop, agent factory, per-turn dispatch, first specialist (GA), per-agent tool mapping, data visualization, multi-step orchestration, and the ADK 2.0 migration (AH-PRDs 01–06, 08–10, 13–15, plus 02b); Skills runtime — sandbox spike + backend + agent-factory integration with SandboxPool (SK-PRDs 00–02); UI design system + core shell + workflows shell + organization selection (UI-PRDs 01, 02, 03, 08 — UI-PRD-03 moved up from R2 to align with AH-PRD-02); Feature Flags (FF-PRDs 01–03); Chat substrate — session metadata + sidebar + categories + todos/artifacts (CH-PRDs 01, 02, 03, 05) |
...
| **3** | Expertise + Monetization | Knowledge Graph migrations + provenance + read tools + session-end automation + research refactor + integration polish (KG-PRDs 01–06); Skills authoring UI + agent-builder controls + predefined-skill foundation (SK-PRDs 03–05); Knowledge/Strategy UI redesign (UI-PRD-05); Billing end-to-end — Stripe, token meter, monthly enforcement, subscription UI, sales handoff (BL-PRDs 01–06) |
...
| **7** | Data Residency | The full US + EU data-residency program — regional-cell foundation + cross-cell admin (DM-PRDs 09–10), agent reasoning + observability residency (AH-PRDs 11–12), and the per-component residency slices (PR-PRD-10, KG-PRD-07, SE-PRD-08, DP-PRD-07, IN-PRD-08, BL-PRD-07, CH-PRD-07); a dedicated milestone gated on EU Agent Engine GA + the EU-launch decision (see DESIGN-REVIEW-LOG Review 46) |
...
|---------|------|--------|---------|
| 4.6 | 2026-06-10 | Development Team | Model-call resilience learnings from the 2026-06-10 staging 429 incident (Review 52, PR #970). §11.1.1 gained a **Model invocation** row (`ResilientGemini` registry-level backoff retry on every Gemini call; genai's `retry_options=None` = never-retry; streaming-initiation caveat) and scoped the existing Agent-invocation row (`agent_retry.py` does not cover model `ClientError`s). New §11.3.3 Vertex Model-Serving Capacity — current Gemini models carry **no per-project quota** (Cloud Quotas API verified); 429 = shared-pool contention; `global` endpoint is the largest pool; remedies are backoff retry → traffic smoothing → Priority PayGo / Provisioned Throughput, with the full options matrix in `docs/spike-vertex-429-long-running-tasks.md`. §11.4 risk matrix gained a "Vertex shared-capacity 429 mid-turn" row (survival math: ≈0.01% → >99.99% for a 2-h task) and corrected the review-loop cost row's stale reviewer model (`gemini-2.0-flash`, discontinued upstream 2026-06-01 → `DEFAULT_REVIEWER_MODEL` `gemini-2.5-pro`). Doc header version/date refreshed (was stale at 4.2/April). |
