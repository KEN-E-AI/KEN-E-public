...
**Isolated AgentTool leaf carry-forward (AH-121 re-plan, §3.3 of AH-PRD-05-trace-contract-diff.md):** A specialist's built-in-tool leaves (`google_search` grounding, `numerical_analyst` code-execution) stay `AgentTool`-isolated — they cannot be task-mode sub-agents (the injected `FinishTaskTool` sibling triggers Gemini `400 "Multiple tools must all be search tools"`). `AgentTool.run_async` drops their inner events (`#3984`), so **MER-E extractors must treat the absence of inner grounded-search / code-exec spans as an accepted `#3984` gap** — the same class as the `google.genai` autopatch carry-forward in §2.1 of the trace-contract-diff. Leaf `usage_metadata` is captured by the AH-PRD-15 `agent_tool_billing.py` off-state sink (keyed by the outer `invocation_id`) and folded into the turn delta; extractors must read it from the drained sink, not from leaf-level events. This gap is locked in CI by `TestIsolatedAgentToolLeafCarryForward` in `test_supervisor_orchestration_fixture.py`. See [AH-PRD-05-trace-contract-diff.md §3.3](design/components/agentic-harness/projects/AH-PRD-05-trace-contract-diff.md) for the full carry-forward detail.

**Retired patterns (§4 of AH-PRD-05-trace-contract-diff.md):** The `execute_workflow` and `invoke_pipeline` span names are retired inner-Runner anti-patterns (the AH-75 defect). Neither name will appear in supervisor traces; extractors that searched for them must remove those lookups. This is locked in CI by `TestRetiredPatterns` in `test_supervisor_orchestration_fixture.py`.
...
## 17. Task-Mode Agent-as-Tool Leaf Spans (AH-PRD-15)

AH-PRD-15 (AgentTool → Task-Mode Migration) migrates `agent.google_search` (and any future
`source="agent"` catalogue entries) off `AgentTool.run_async` to `mode='task'` sub-agent
dispatch on ADK 2.0. The motivation: GitHub `google/adk-python#3984` means `AgentTool.run_async`
silently discards inner sub-agent events (including `usage_metadata` and grounded-search tool
spans) on 2.0 — the same `AH-75` billing/tracing defect, now on the most critical agents.
This section documents the **post-migration trace shape** MER-E extractors must consume.

### 17.1 Span Hierarchy

The task-mode dispatch introduces two new span types inside the specialist worker's subtree:

```
root agent (ken_e)                                                        [EMITTED]
└── transfer_to_agent [summary.agent_name = "google_analytics_specialist"] [EMITTED]
└── google_analytics_specialist (loop_pipeline specialist)                 [EMITTED]
    └── google_analytics_specialist_worker                                  [EMITTED]
        └── request_task_<name> tool span                                  [EMITTED]
            └── <name>_agent task-mode leaf span                           [EMITTED]
                summary:
                  agent_kind: "task_mode"
                  task_id: str            # correlation key
                  node_path: str          # ADK 2.0 path per §17.2
                children:
                  └── google_search (grounded-search tool call #1)         [EMITTED]
                  └── google_search (grounded-search tool call #2, ...)    [EMITTED]
                  └── generate_content (google.genai LLM call)             [DEFERRED — see §4.5]
```

**Key observation (the fix this migration delivers):** before AH-PRD-15, the
`google_search` grounded-search tool spans under the `google_search_agent` leaf were
invisible in the outer stream (`#3984`). After the migration, they appear as `[EMITTED]`
spans. This is the primary observability signal verified by AH-118.

> **Emission status.** Only the spans tagged `[EMITTED]` above are written by the
> AH-PRD-15 runtime. The `generate_content` span inside the task-mode leaf is `[DEFERRED]`
> — the Weave autopatch carry-forward on ADK 1.34.1 and 2.0 (see §4.5). Treat deferred
> spans and fields as absent in live traces until the autopatch is fixed.

### 17.2 Per-Span Attributes

#### `request_task_<name>` tool span

| Attribute | Type | Source | Notes |
|---|---|---|---|
| `tool_name` | `str` | ADK native | E.g. `"request_task_google_search"` — auto-generated by ADK for each `mode='task'` sub-agent |
| `task_id` | `str` | ADK native | Unique task identifier; matches the task-mode leaf's `summary.task_id` |
| `status` | `str` | ADK native | `"success"` on normal completion |

This span is emitted natively by ADK's task-mode dispatch mechanism. No custom attributes
are written to it by KEN-E.

#### Task-mode leaf span (`<name>_agent`)

The span `name` equals the leaf agent's ADK name (e.g. `"google_search_agent"`). Written
by the `after_agent_callback` installed on the leaf.

| Attribute | Type | Source | Notes |
|---|---|---|---|
| `agent_kind` | `str` | `after_agent_callback` | Always `"task_mode"` for task-mode leaves |
| `task_id` | `str` | `after_agent_callback` | Matches the parent `request_task_<name>` span's `task_id` |
| `node_path` | `str` | ADK 2.0 event `node_info` | Format: `<parent_agent>@<index>/task_specialist@adk-<uuid>` — sourced from AH-99 probe-1 evidence (`docs/spike-adk2-supervisor-orchestration-live.md` §4). For a specialist-dispatched task: `google_analytics_specialist@1/task_specialist@adk-<uuid>`. For a root-dispatched task: the coordinator/root segment replaces the specialist prefix. |

#### Inner `google_search` grounded-search tool spans

These are the primary observability signal for AH-PRD-15 §7.2 AC #2.

| Attribute | Type | Source | Notes |
|---|---|---|---|
| `tool_name` | `str` | `after_tool_callback` | `"google_search"` |
| `status` | `str` | `after_tool_callback` | `"success"` on normal completion |
| `input` | `dict` | `before_tool_callback` | Full (unsummarized) tool input per §6. Key field: `"query"` |
| `output` | `dict` | `after_tool_callback` | Full (unsummarized) tool output per §6. Key field: `"results"` (list) |

Per §6, full (unsummarized) `input` and `output` are required on tool call spans for MER-E
Result Utilization and Query Quality evaluation. The `google_search` tool spans inside the
task-mode leaf satisfy this requirement.

#### `generate_content` span (inside task-mode leaf)

**Deferred** — see §4.5 (Weave autopatch carry-forward, pre-existing on both ADK 1.34.1 and
2.0). MER-E must not key required extractor logic on this span; treat its absence as expected
until a future Weave or `google.genai` release fixes the autopatch. The canonical fixture
marks this span `emission_status='deferred'` with the carry-forward reason; the conformance
test `test_google_search_task_mode_fixture.py::TestEmissionStatus::test_generate_content_span_is_deferred`
locks this classification in CI.

### 17.3 Relation to §14 Span Hierarchy

The `request_task_<name>` tool span and the task-mode leaf agent span appear inside the
`{specialist_name}_worker` span, at the **L4** position (children of the L3 worker). The
grounded-search `google_search` tool spans inside the leaf are at the **L5** position.

This nesting is deeper than the pre-migration `AgentTool` path because task-mode dispatch
preserves the full call tree rather than discarding inner events.

### 17.4 MER-E Extractor Guidance

1. **Match `request_task_<name>` by prefix:** `op_name.startswith("request_task_")`. The
   suffix is the bare tool name from the `source="agent"` catalogue entry (e.g.
   `request_task_google_search`).

2. **Match the task-mode leaf agent span by `summary.agent_kind == "task_mode"`** — this
   is the stable discriminator. The span `name` (e.g. `google_search_agent`) also uniquely
   identifies it but `agent_kind` is more robust to future renames.

3. **Extract grounded-search tool calls using `op_name == "tool:google_search"` or
   `tool_name == "google_search"`** per §3 conventions. These are the `[EMITTED]` signals
   that prove the fix works; key extraction on them for quality scoring.

4. **`task_id` is the correlation key** between the `request_task_<name>` span and the
   task-mode leaf. Correlate by `task_id` to join task dispatch → leaf execution → inner
   tool calls in a single logical unit.

5. **`node_path` format for task-mode leaf**: `<parent_agent>@<index>/task_specialist@adk-<uuid>`.
   The first segment identifies the dispatching agent (specialist or root/coordinator);
   the second segment identifies the task node instance. Validate against `_NODE_PATH_RE`
   from `test_supervisor_orchestration_fixture.py` and `test_google_search_task_mode_fixture.py`.

6. **Honour `generate_content` deferred status** — it is absent in live traces per §4.5.
   Do not build required scoring on it.

7. **Validate against the canonical fixture** at
   `app/adk/tracking/tests/fixtures/google_search_task_mode_trace.json` and run
   `app/adk/tracking/tests/test_google_search_task_mode_fixture.py`.

### 17.5 Fixture Pointer

```
app/adk/tracking/tests/fixtures/google_search_task_mode_trace.json
```

The fixture covers the specialist-assignment path (AH-115) as the baseline. The root-
assignment path (AH-116) is architecturally identical at the leaf level — only the parent
context (`ken_e` root vs. `google_analytics_specialist`) and the first `node_path` segment
differ. See the fixture `description` field for the D-A2 rationale.

Live-verification runbook: `docs/runs/AH-118-google-search-trace-coverage.md`. PRD:
`docs/design/components/agentic-harness/projects/AH-PRD-15-agenttool-migration-cutover.md`
§7.2 (AC #2) + §8 (Test Plan — Trace).
