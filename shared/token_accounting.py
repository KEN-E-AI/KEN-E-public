...
class BillableTokenCounts(BaseModel):
    ...
    input: int = Field(
        default=0, ge=0, description="Prompt tokens minus cached-input tokens"
    )
    output: int = Field(
        default=0, ge=0, description="Completion tokens generated by the model"
    )
    reasoning: int = Field(
        default=0,
        ge=0,
        description="Internal reasoning tokens (0 for non-reasoning models)",
    )
