...
export const useDialogFix = () => {
  ...
  (window as any).fixFrozenUI = () => {
    ...
    document
      .querySelectorAll(
        '[role="dialog"], [data-state="open"], [aria-hidden="true"]',
      )
      .forEach((el) => {
        if (
          el.getAttribute("data-radix-portal") ||
  ...
  useEffect(() => {
    ...
    const cleanupDialogs = () => {
      ...
      if (bodyStyle.pointerEvents === "none") {
        console.warn(
          "[useDialogFix] Body pointer-events were disabled, resetting...",
        );
      ...
      openElements.forEach((el) => {
        ...
        if (!isVisible) {
          console.warn(
            "[useDialogFix] Found hidden element with data-state='open', removing...",
            el,
          );
          el.remove();
        }
