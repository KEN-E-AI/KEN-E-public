...
export function useAccountCreationProgress(
  ...
  useEffect(() => {
    console.log(
      "[useAccountCreationProgress] Hook called with accountId:",
      accountId,
    );
    if (!accountId) {
    ...
    const checkStatus = async () => {
      ...
      if (Date.now() - startTimeRef.current > maxPollingTime) {
        console.log(
          "[useAccountCreationProgress] Max polling time exceeded, stopping",
        );
        if (intervalId) {
        ...
        setProgress({
          status: "failed",
          message:
            "Account creation is taking longer than expected. Please refresh the page.",
        });
        return;
      }
      ...
      try {
        ...
        if (response.data) {
          console.log(
            "[useAccountCreationProgress] Status received:",
            response.data,
          );
          ...
          if (
            ...
            console.log(
              "[useAccountCreationProgress] Creation finished, stopping polling",
            );
            if (intervalId) {
              clearInterval(intervalId);
            }
    ...
    console.log(
      "[useAccountCreationProgress] Started polling every 30 seconds",
    );
