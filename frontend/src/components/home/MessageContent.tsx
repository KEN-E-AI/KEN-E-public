...
function parseMessageContent(content: string): ParsedContent {
    ...
    if (content[currentPos] === '{') {
      ...
      if (endPos !== -1) {
        ...
        try {
          ...
          if (jsonObj.function_call || jsonObj.function_response || 
              jsonObj.tool_calls || jsonObj.type === 'function') {
  ...
  if (jsonObjects.length > 0) {
    ...
    const combinedJson = jsonObjects.length === 1 
      ? jsonObjects[0] 
      : { debug_data: jsonObjects };
  ...
  if (jsonStartIndex >= 0 && jsonPart) {
    try {
      ...
      if (jsonData.function_call || jsonData.function_response || 
          jsonData.tool_calls || jsonData.type === 'function' ||
          jsonData.name || jsonData.arguments) {
        return {
...
export function MessageContent({ content, className, isAssistant = false }: MessageContentProps) {
  ...
  if (!parsedContent.jsonData) {
    return (
      <div className={cn("text-sm leading-relaxed prose prose-sm max-w-none", 
        ...
        <ReactMarkdown 
          ...
          components={{
            ...
            thead: ({ children }) => (
              <thead className={cn(
                isAssistant ? "bg-white/10" : "bg-gray-50"
              )}>
                {children}
              </thead>
            ),
            tbody: ({ children }) => (
              <tbody className={cn(
                "divide-y",
                isAssistant ? "divide-white/10 bg-white/5" : "divide-gray-200 bg-white"
              )}>
                {children}
              </tbody>
            ),
  ...
  const displayText = parsedContent.text || (parsedContent.jsonData ? 'Response received' : '');
