...
async def test_neo4j_account_creation():
    ...
    try:
        ...
        if result:
            logger.info(
                f"✅ Account created successfully: {result[0]['acc']['account_id']}"
            )
        else:
    ...
    try:
        ...
        result = await neo4j_service.execute_query(
            fetch_query, {"account_id": account_id}
        )

        if result:
            logger.info(
                f"✅ Account fetched successfully: {result[0]['acc']['account_name']}"
            )
        else:
    ...
    try:
        ...
        if result:
            ...
            for i, record in enumerate(result[:3]):
                logger.info(
                    f"  - {record['acc']['account_name']} ({record['acc']['account_id']})"
                )
        else:
