...
async def create_initial_activity_logs(account_id: str, industry: str, db: Optional[BigQueryService]) -> int:
    """Stub for creating initial activity logs."""
    if not db:
        return 0
    logger.info(f"[ACCOUNT_CREATION] Creating initial activity logs for {account_id} in {industry}")
    return 0
...
async def create_account_internal(
    ...
    if bigquery_service:
        try:
            ...
            logger.info(f"[ACCOUNT_CREATION] Created {created_count} initial activity logs")
        except Exception as e:
    ...
    try:
        ...
        logger.info(f"Ensured GCS bucket {bucket_name} exists in {location} for account {account_id}")
    ...
    try:
        ...
        logger.info(f"Created Firestore collection '{collection_name}' with placeholder document: {doc_id}")
    except Exception as e:
        logger.error(f"Failed to create Firestore collection for account {account_id}: {e}")
    ...
    try:
        ...
        if success:
            logger.info(f"Granted admin permissions to user {user.user_id} for account {account_id}")
        else:
            logger.warning(f"Failed to grant permissions to user {user.user_id} for account {account_id}")
    except Exception as e:
        logger.error(f"Error granting permissions to user {user.user_id} for account {account_id}: {e}")
