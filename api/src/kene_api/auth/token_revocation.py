...
class TokenRevocationService:
    ...
    async def revoke_all_user_tokens(
        ...
        db.collection(f"{self.collection_name}_all").document(user_id).set(revocation_data)
    ...
    async def is_token_revoked(self, token_id: str, user_id: str, issued_at: Optional[float] = None) -> bool:
        ...
        # Check if all user tokens were revoked
        if issued_at:
            all_tokens_doc = db.collection(f"{self.collection_name}_all").document(user_id).get()
    ...
    async def cleanup_expired_revocations(self) -> int:
        ...
        now = datetime.utcnow()
        expired_docs = db.collection(self.collection_name).where(
            "expires_at", "<", now
        ).stream()
