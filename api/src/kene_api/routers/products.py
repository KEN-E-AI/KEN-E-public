    ...
    try:
        ...
        for dataset_name, dataset_config in datasets_dict.items():
            ...
            if "dataset_id" not in dataset_config:
                import hashlib
                dataset_config["dataset_id"] = int(hashlib.md5(dataset_name.encode()).hexdigest()[:8], 16)
        ...
        for dataset_config in datasets:
            try:
                ...
                dataset_request = DatasetRequest(
                    ...
                    products=[request.product],  # Add the current product to the products array
                ...
                if dataset_response.success:
                    ...
                    for metric_config in metrics:
                        ...
                        if isinstance(metric_config.get("account_components"), str):
                            import ast
                            try:
                                metric_config["account_components"] = ast.literal_eval(metric_config["account_components"])
                        ...
                        try:
                            ...
                            metric_request = MetricRequest(
                                ...
                                account_components=metric_config.get("account_components"),
                            ...
                            # Create metric
                            metric_response = await create_metric(metric_request, db, superset_client)
                            if metric_response.success:
                                created_metrics.append({
                                    "metric_id": metric_response.data.get("metric_id"),
                                    "metric_name": metric_config.get("metric_name"),
                                    "dataset_id": dataset_config.get("dataset_id")
                                })
                            else:
                                errors.append(f"Failed to create metric '{metric_config.get('metric_name')}': {metric_response.message}")
                                
                        except Exception as e:
                            logger.error(f"Error creating metric {metric_config.get('metric_name')}: {e}")
                            errors.append(f"Error creating metric '{metric_config.get('metric_name')}': {str(e)}")
                            
                else:
                    errors.append(f"Failed to create dataset '{dataset_config.get('dataset_name')}': {dataset_response.message}")
                    
            except Exception as e:
                logger.error(f"Error processing dataset {dataset_config.get('dataset_name')}: {e}")
                errors.append(f"Error processing dataset '{dataset_config.get('dataset_name')}': {str(e)}")
        ...
        for record in datasets_result:
            ...
            try:
                ...
                if delete_response.success:
                    ...
                    if "deleted" in delete_response.message and "related metrics" in delete_response.message:
                        import re
                        metrics_match = re.search(r'deleted (\d+) related metrics', delete_response.message)
                        if metrics_match:
                            total_metrics_deleted += int(metrics_match.group(1))
                else:
                    errors.append(f"Failed to delete dataset '{dataset_name}': {delete_response.message}")
