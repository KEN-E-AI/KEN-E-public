...
_template_cache = ThreadSafeTemplateCache(max_size=CACHE_MAX_SIZE, ttl_seconds=CACHE_TTL_SECONDS)
...
def _parse_template_data(template_dict: dict[str, Any]) -> IndustryTemplate:
    ...
    recommended_settings = template_dict.get("recommended_settings", {})
    if not isinstance(recommended_settings, dict):
        recommended_settings = {}
    
    # Ensure required fields for recommended_settings
    settings = IndustryTemplateSettings(
        timezone=recommended_settings.get("timezone", "America/New_York"),
        data_region=recommended_settings.get("data_region", "United States"),
        industry=recommended_settings.get("industry", template_dict.get("industry", ""))
    )
    
    # Handle default settings
    default_settings = template_dict.get("default_settings", {})
    if not isinstance(default_settings, dict):
        default_settings = {}
    
    defaults = IndustryTemplateDefaults(
        data_retention=default_settings.get("data_retention", 90)
    )
    ...
    return IndustryTemplate(
        ...
        definition=template_dict.get("definition", ""),
        default_objectives=template_dict.get("default_objectives", []),
        default_channels=template_dict.get("default_channels", []),
        default_kpis=template_dict.get("default_kpis", []),
        marketing_channels=template_dict.get("marketing_channels", template_dict.get("default_channels", [])),
        product_integrations=template_dict.get("product_integrations", []),
        recommended_settings=settings,
        default_settings=defaults,
        is_active=template_dict.get("is_active", True),
        created_at=template_dict.get("created_at", datetime.now(timezone.utc).isoformat()),
        updated_at=template_dict.get("updated_at", datetime.now(timezone.utc).isoformat())
    )


async def _fetch_all_templates(firestore_service: FirestoreService) -> list[IndustryTemplate]:
    """
    ...
    for template_dict in templates_data:
        try:
            ...
            logger.warning(f"Failed to parse template {template_dict.get('id', 'unknown')}: {e}")
            continue
...
@router.get("/industry-templates", response_model=IndustryTemplateListResponse)
async def list_industry_templates(
    ...
    **Example:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to fetch industry templates"
        )


@router.get("/industry-templates/industry/{industry}", response_model=IndustryTemplate)
async def get_industry_template(
    industry: str = Path(..., description="Industry name", min_length=1, max_length=200),
    firestore_service: FirestoreService = Depends(get_firestore_service),
) -> IndustryTemplate:
    """
    ...
    **Example:**
    ...
    try:
        ...
        if template is None:
            raise HTTPException(
                status_code=404,
                detail=f"Template not found for industry: {industry}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch template for industry: {industry}"
        )


@router.get("/industry-templates/{template_id}", response_model=IndustryTemplate)
async def get_template_by_id(
    template_id: str = Path(..., description="Template ID", min_length=1, max_length=200),
    firestore_service: FirestoreService = Depends(get_firestore_service),
) -> IndustryTemplate:
    """
    ...
    **Example:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch template: {template_id}"
        )


@router.put("/industry-templates/{template_id}", response_model=IndustryTemplate)
async def update_industry_template(
    ...
    **Example:**
    ...
    if not user.is_super_admin:
        raise HTTPException(
            status_code=403,
            detail="Only super admins can update industry templates"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update template: {template_id}"
        )
