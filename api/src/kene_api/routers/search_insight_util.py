...
def superset_login():
    ...
    response = requests.post(f"{os.getenv("SUPERSET_BASE_URL")}/api/v1/security/login", json=json)
    ...
    response = requests.get(f"{os.getenv("SUPERSET_BASE_URL")}/api/v1/security/csrf_token/",
...
def calculate_metric_movement(metric, database_id, dataset_name, date_col_name, activity):
    ...
    response = requests.post(f"{os.getenv("SUPERSET_BASE_URL")}/api/v1/sqllab/execute/", json=payload,
...
def main(account_id,
    ...
    try:
        ...
        for record in known_activities:
            ...
            record['a']['active_evidence']['evidence'] = [record['al']['activity_log_id']]
            all_activities[record['a']['activity_id']] = record['a']
            related_metrics = query_neo4j(INFLUENCE_LIKELY_ACTIVITIES_QUERY.format(activity_id=record['a']['activity_id']))
            ...
            for metric_record in related_metrics:
                if metric_record['m']['metric_id'] == input_metric_id:
                    ...
                    if metric_record['influence_direction'] == input_direction:
                        record['a']['influence_evidence']['influence_direction_aligned'] = True
                    else:
                        record['a']['influence_evidence']['influence_direction_aligned'] = False
        ...
        for record in unknowable_activities:
            ...
            if 'al' in record:
                ...
                log = record['al']
                log_sd, log_ed = \
                    datetime.strptime(log['start_date'], '%Y-%m-%d').date(), datetime.strptime(log['end_date'], '%Y-%m-%d').date()
                if log_sd <= comparison_start_date <= log_ed or \
                log_sd <= comparison_end_date <= log_ed:
                    ...
                    record['a']['comparison_start_date'] = comparison_start_date - relativedelta(weeks=1)
            ...
            print(f'Checking for metrics likely influenced by activity {record["a"]["activity_id"]}')
            related_metrics = query_neo4j(INFLUENCE_LIKELY_ACTIVITIES_QUERY.format(activity_id=record['a']['activity_id']))
            # print(f"Related metrics for activity:")
            record['a']['influence_evidence']['influence_likely'] = False
            for metric_record in related_metrics:
                ...
                record['a']['active_evidence']['data'][metric_record['m']['metric_name']] = metric_record['m']
                record['a']['active_evidence']['data']['evaluation_metric_value'] = evaluation_metric_value
                record['a']['active_evidence']['data']['comparison_metric_value'] = comparison_metric_value
        ...
        for activity_id, activity in all_activities.items():
            ...
            for insight in insights:
                ...
                log_sd, log_ed = \
                    datetime.strptime(activity_log['start_date'], '%Y-%m-%d').date(), datetime.strptime(activity_log['end_date'], '%Y-%m-%d').date()
                if influence_type == 'INFLUENCE_CONFIRMED' and influence_confirmed_direction == SOURCE_DIRECTION and \
                (log_sd <= evaluation_start_date <= log_ed or \
                log_sd <= evaluation_end_date <= log_ed):
                    overlapping_supporting_insights.append(activity_log)
                elif (influence_type == 'NO_INFLUENCE_CONFIRMED' or influence_confirmed_direction != SOURCE_DIRECTION) and \
                (log_sd <= evaluation_start_date <= log_ed or \
                log_sd <= evaluation_end_date <= log_ed):
                    overlapping_conflicting_insights.append(activity_log)
                elif influence_type == 'INFLUENCE_CONFIRMED' and influence_confirmed_direction == SOURCE_DIRECTION and \
                not (log_sd <= evaluation_start_date <= log_ed or \
                log_sd <= evaluation_end_date <= log_ed):
            ...
            activity['influence_evidence']['overlapping_supporting_insights'] = overlapping_supporting_insights
            activity['influence_evidence']['overlapping_conflicting_insights'] = overlapping_conflicting_insights 
            activity['influence_evidence']['other_supporting_insights'] = other_supporting_insights
            activity['influence_evidence']['other_conflicting_insights'] = other_conflicting_insights
            ...
            if len(overlapping_supporting_insights) > 0 and len(overlapping_conflicting_insights) > 0:
                 activity['influence_evidence']['overlapping_supporting_insight_ratio'] = len(overlapping_supporting_insights) / (len(overlapping_supporting_insights) + len(overlapping_conflicting_insights))
            if len(other_supporting_insights) > 0 and len(other_conflicting_insights) > 0:
                activity['influence_evidence']['overlapping_supporting_insight_ratio'] = len(overlapping_supporting_insights) / (len(overlapping_supporting_insights) + len(overlapping_conflicting_insights))
