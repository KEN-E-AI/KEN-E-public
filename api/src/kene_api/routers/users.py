...
class UserResponse(BaseModel):
    ...
    email: str
    profile: UserProfile
    permissions: dict[str, dict[str, str]] = Field(default_factory=lambda: {"accounts": {}, "organizations": {}})
...
async def get_current_user(
    ...
    if not user_doc.exists:
        ...
        firestore_db.collection("users").document(current_user.user_id).collection("notifications").document("settings").set(default_notifications.dict())
...
async def get_notification_settings(
    ...
    settings_doc = firestore_db.collection("users").document(current_user.user_id).collection("notifications").document("settings").get()
...
async def update_notification_settings(
    ...
    firestore_db.collection("users").document(current_user.user_id).collection("notifications").document("settings").set(settings.dict())
...
async def get_security_settings(
    ...
    settings_doc = firestore_db.collection("users").document(current_user.user_id).collection("security").document("settings").get()
...
async def update_security_settings(
    ...
    firestore_db.collection("users").document(current_user.user_id).collection("security").document("settings").set(settings.dict())
