...
async def trigger_strategy_generation(
    ...
    user_context: Optional[Any] = None  # Allow passing existing UserContext for chat-triggered calls
) -> None:
    """
...
Use the execute_strategy_generation tool to generate all 5 strategy documents."""
        ...
        try:
            ...
            if user_context:
                ...
                if response_content:
                    ...
                    logger.error(f"Strategy generation returned empty response for {company_name}")
                ...
                project_id = os.getenv("VERTEX_AI_PROJECT_ID", os.getenv("GOOGLE_CLOUD_PROJECT_ID"))
                ...
                logger.info(f"Calling agent engine with message length: {len(message)} chars")
                ...
                for chunk in response:
                    ...
                    if isinstance(chunk, dict):
                        logger.info(f"Chunk {chunk_count} is dict with keys: {list(chunk.keys())[:10]}")
                        
                        # Log the full chunk for debugging
                        import json
                        try:
                            logger.debug(f"Full chunk {chunk_count}: {json.dumps(chunk, default=str)[:500]}")
                        except:
                            logger.debug(f"Chunk {chunk_count} (non-serializable): {str(chunk)[:500]}")
                            ...
                            if 'parts' in content and isinstance(content['parts'], list):
                                for part in content['parts']:
                                    if isinstance(part, dict) and 'text' in part:
                                        ...
                                        logger.info(f"  Added text part from chunk {chunk_count}: {len(part['text'])} chars")
                                        logger.debug(f"  Text preview: {part['text'][:200]}...")
                        else:
                            response_parts.append(str(chunk))
                            logger.info(f"  Added chunk {chunk_count} as string: {len(str(chunk))} chars")
                    else:
                        response_parts.append(str(chunk))
                        logger.info(f"Chunk {chunk_count} type: {type(chunk).__name__}, size: {len(str(chunk))}")
                
                logger.info(f"✅ Received total {chunk_count} chunks from agent engine")
                
                result = ''.join(response_parts).strip()
                if result:
                    logger.info(f"Strategy generation completed for {company_name}: {len(result)} chars")
        ...
        logger.info(f"Strategy generation result preview: {result[:500] if result else 'Empty'}...")
        ...
        logger.error(f"Failed to generate strategy documents for account {account_id}: {e}")
...
async def verify_strategy_documents_created(account_id: str) -> bool:
    ...
    try:
        ...
        for doc_type in expected_docs:
            ...
            if doc.exists:
                ...
                if isinstance(content, dict):
                    ...
                    logger.info(f"Found {doc_type} document - size: {content_size} bytes, keys: {has_keys}, status: {doc_data.get('status', 'unknown')}")
                else:
                    logger.warning(f"Found {doc_type} but content is not a dict: {type(content).__name__}")
        
        # Log summary
        if doc_quality:
            logger.info(f"Document quality summary for {account_id}:")
            for doc_type, quality in doc_quality.items():
                logger.info(f"  - {doc_type}: {quality['size']} bytes, {quality['keys']} keys, v{quality['version']}")
        
        if len(found_docs) == len(expected_docs):
            logger.info(f"✅ All {len(expected_docs)} strategy documents found for account {account_id}")
            return True
        elif len(found_docs) > 0:
            logger.warning(f"⚠️ Only {len(found_docs)}/{len(expected_docs)} strategy documents found for account {account_id}")
            logger.warning(f"  Found: {found_docs}")
            logger.warning(f"  Missing: {[d for d in expected_docs if d not in found_docs]}")
        ...
        logger.error(f"Failed to verify strategy documents for account {account_id}: {e}")
        return False
