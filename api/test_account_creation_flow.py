...
class AccountCreationTester:
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(1800.0)) as client:  # 30 min timeout
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log(f"Account created successfully: {self.account_id}", "SUCCESS")
                    return self.account_id
                else:
                    self.log(f"Account creation failed: {response.status_code}", "ERROR")
    ...
    async def monitor_account_creation(self, account_id: str, max_duration: int = 1800):
        ...
        self.log(f"Monitoring account creation for up to {max_duration/60:.0f} minutes...")
    ...
    async def list_accounts(self, organization_id: str):
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(360.0)) as client:  # 6 min timeout
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log(f"Successfully fetched {len(accounts)} accounts in {list_duration:.1f}s", "SUCCESS")
                    
                    # Check if our new account is in the list
                    if self.account_id:
                        found = any(acc.get("account_id") == self.account_id for acc in accounts)
                        if found:
                            self.log(f"New account {self.account_id} found in list", "SUCCESS")
                        else:
                            self.log(f"New account {self.account_id} NOT found in list", "WARNING")
                else:
                    self.log(f"List accounts failed: {response.status_code} after {list_duration:.1f}s", "ERROR")
                    
            except httpx.TimeoutException as e:
                list_duration = time.time() - list_start
                self.log(f"List accounts timed out after {list_duration:.1f}s", "ERROR")
                self.log(f"This is the 5-minute timeout issue!", "ERROR")
            except Exception as e:
                self.log(f"List accounts error: {str(e)}", "ERROR")
    ...
    async def run_full_test(self):
        ...
        self.log("=" * 60)
        self.log(f"TEST COMPLETED - Total duration: {total_duration:.1f}s ({total_duration/60:.1f} minutes)")
        self.log("=" * 60)
