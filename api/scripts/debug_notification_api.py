...
async def debug_notifications():
    ...
    try:
        ...
        print(
            f"      Categories: {prefs.categories[:3]}... ({len(prefs.categories)} total)"
        )
    ...
    try:
        notifications = await notification_service.get_user_notifications(
            user_id=user_id, account_ids=account_ids, include_archived=False
        )
        ...
        for notif in notifications[:3]:  # Show first 3
            archived_at = (
                datetime.fromisoformat(
                    notif.notification.archived_at.replace("Z", "+00:00")
                )
                if notif.notification.archived_at
                else None
            )
    ...
    try:
        all_notifications = await notification_service.get_user_notifications(
            user_id=user_id, account_ids=account_ids, include_archived=True
        )
        print(
            f"   Found {len(all_notifications)} total notifications (including archived)"
        )
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    raw_notifications = notifications_ref.where(
