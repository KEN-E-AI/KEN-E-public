...
def debug_by_email(email: str, fs: FirestoreService):
    ...
    if not users:
        ...
        if invitations:
            ...
            for inv in invitations:
                ...
                print(f"  Organization: {inv.get('organization_name')} ({inv.get('organization_id')})")
                print(f"  Access Level: {inv.get('access_level')}")
                print(f"  Status: {inv.get('status')}")
                print(f"  Account Permissions: {inv.get('account_permissions', 'None')}")
        else:
    ...
    if not account_perms and org_perms:
        ...
        print("This means accessible_accounts returns [] even though user has access via org role")
