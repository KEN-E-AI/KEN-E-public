...
async def simulate_account_creation_and_notification():
    ...
    logger.info("\n1. USER CONTEXT:")
    logger.info(
        f"   - Is org admin: {user_context.organization_permissions.get(test_org_id) == 'admin'}"
    )
    logger.info(
        f"   - Accessible accounts (explicit): {user_context.accessible_accounts}"
    )
    logger.info(
        f"   - Has account access (should be True for org admin): {user_context.has_account_access(test_account_id)}"
    )
    ...
    try:
        ...
        logger.info("   ✓ Created notification status for user")
    except Exception as e:
    ...
    if not user_context.accessible_accounts:
        logger.info(
            "   - User has no explicit account permissions (expected for org admin)"
        )

        # Check if user is org admin
        if any(
            role == "admin" for role in user_context.organization_permissions.values()
        ):
    ...
    if status_doc.exists:
        logger.info("   ✓ User status exists in Firestore")
        ...
        logger.error("   ✗ User status NOT found in Firestore")

    logger.info("\n=== SIMULATION COMPLETE ===")

    # Cleanup
    try:
        firestore.get_client().collection("notifications").document(
            notification_id
        ).delete()
