...
async def test_conversation_persistence():
    ...
    try:
        ...
        test_messages_2 = [
            ...
            ChatMessage(role="user", content="What about their recent financial performance?")
        ]
        ...
        try:
            history = await client.get_conversation_history(test_user.user_id, session_id)
            if history:
        ...
        print(f"✅ Sent message to second conversation")
        print()
        ...
        print(f"✅ Session ID consistency: {'PASS' if session_id == returned_session_id else 'FAIL'}")
        print(f"✅ Conversation listing: {'PASS' if test_conversation else 'FAIL'}")
        print(f"✅ Multiple conversations: {'PASS' if len(found_sessions) == 2 else 'FAIL'}")
        print(f"✅ History retrieval: {'PASS' if history else 'FAIL'}")
        
        if all([session_id, response, session_id == returned_session_id, test_conversation, len(found_sessions) == 2]):
