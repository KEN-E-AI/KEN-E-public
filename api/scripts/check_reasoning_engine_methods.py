...
try:
    ...
    try:
        ...
        print("✅ Successfully imported vertexai and reasoning_engines")
    ...
    if not agent_engine_id:
        ...
        print(
            f"🔍 ReasoningEngine class MRO: {reasoning_engines.ReasoningEngine.__mro__}"
        )
        ...
        class_methods = [
            method
            for method in dir(reasoning_engines.ReasoningEngine)
        ...
        if has_agent_engines:
            ...
            agent_engine_attrs = [
                attr for attr in dir(agent_engines) if not attr.startswith("_")
            ]
    ...
    print(
        f"📊 Total public attributes: {len([m for m in methods if not callable(getattr(reasoning_engine, m))])}"
    )
