...
async def test_notification_creation():
    ...
    try:
        ...
        if notification:
            logger.info("✓ Notification verified in database")
        ...
        if notification_id in user_statuses:
            ...
            logger.info("✓ User status verified")
        ...
        if notifications:
            logger.info("✓ Lazy loading works - other user can see notification")
        ...
        try:
            ...
            firestore.get_client().collection("users").document(
                test_user_id
            ).collection("notification_status").document(notification_id).delete()
