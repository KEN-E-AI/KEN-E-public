...
def main():
    ...
    logger.info(
        f"SENDGRID_API_KEY: {'SET (Secret Manager path)' if sendgrid_key.startswith('projects/') else sendgrid_key[:20] if sendgrid_key != 'NOT SET' else 'NOT SET'}"
    )
    ...
    if email_service.app_base_url == "http://localhost:8080":
        logger.warning(
            "⚠ APP_BASE_URL is using localhost - recipients will get a localhost link!"
        )
    ...
    if test_email and "@" in test_email:
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    "  Check inbox for email with subject: You've been invited to join Test Organization on KEN-E"
                )
