...
def check_notifications():
    ...
    if notif_count == 0:
        ...
        for doc in recent:
            data = doc.to_dict()
            print(
                f"  - ID: {doc.id}, Account: {data.get('account_id')}, Category: {data.get('category')}"
            )
    else:
    ...
    for user_info in users_to_check:
        ...
        if user_doc.exists:
            ...
            print(
                f"  Has notification preferences: {'✅' if pref_doc.exists else '❌'}"
            )
            ...
            print(
                f"  Should see notifications: {'✅' if can_see_notifications else '❌'}"
            )
        else:
            print(f"\n{user_info['email']}:")
            print("  User exists: ❌")
