...
async def test_account_creation():
    ...
    user = UserContext(
        ...
        accounts={},
        permissions={"organizations": {"test_org_123": "admin"}, "accounts": {}},
    )
    ...
    try:
        ...
        if elapsed > 30:
            logger.warning(
                f"⚠️ Account creation took longer than 30 seconds ({elapsed:.2f}s)"
            )
        else:
            logger.info(
                f"✅ Account creation completed within acceptable time ({elapsed:.2f}s < 30s)"
            )
