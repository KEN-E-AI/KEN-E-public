...
def test_local_invitation():
    ...
    test_email = input(
        "\nEnter email address to send test invitation (or press Enter to skip): "
    ).strip()
    ...
    if test_email and "@" in test_email:
        ...
        if result:
            ...
            logger.info("  Check your inbox for the invitation")
        else:
...
def check_deployed_api():
    ...
    try:
        ...
        if health_response.status_code == 200:
            logger.info("✓ Staging API is healthy")
        else:
            logger.warning(
                f"⚠ Staging API returned status {health_response.status_code}"
            )
...
def test_sendgrid_api_directly():
    ...
    test_email = input(
        "\nEnter email for direct SendGrid test (or press Enter to skip): "
    ).strip()
    ...
    if test_email and "@" in test_email:
        try:
            ...
            logger.info("✓ Email sent directly via SendGrid!")
...
if __name__ == "__main__":
    ...
    print(
        '  gcloud logging read \'resource.type="cloud_run_revision" AND resource.labels.service_name="kene-api-staging" AND textPayload:"SendGrid"\' --limit=50 --project=ken-e-staging'
    )
    print("=" * 60)
