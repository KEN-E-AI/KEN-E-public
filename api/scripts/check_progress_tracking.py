...
async def test_progress_storage():
    ...
    for percentage, message in stages:
        ...
        progress = AccountCreationProgress(
            ...
            steps=[
                ProgressStep(
                    name=msg,
                    status="completed"
                    if stages.index((pct, msg)) < stages.index((percentage, message))
                    else "processing"
                    if stages.index((pct, msg)) == stages.index((percentage, message))
                    else "pending",
                )
        ...
        if retrieved:
            print(
                f"  Retrieved: {retrieved.get('percentage')}% - {retrieved.get('message')}"
            )
        else:
    ...
    if final_progress:
        print("✓ Final progress retrieved successfully:")
...
async def test_progress_endpoint():
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print(
                "❌ Could not connect to API server. Make sure it's running on port 8000"
            )
