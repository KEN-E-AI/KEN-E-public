...
async def populate_industry_keywords():
    ...
    for industry, keywords in INDUSTRY_KEYWORDS.items():
        try:
            ...
            doc_id = industry.lower().replace(" ", "_").replace(",", "").replace("(", "").replace(")", "")
            ...
            error_count += 1
            print(f"✗ Error creating keywords for {industry}: {str(e)}")
    ...
    if success_count > 0:
        ...
        try:
            sample_doc = db.collection("industry_keywords").document("manufacturing").get()
            if sample_doc.exists:
                data = sample_doc.to_dict()
                print(f"✓ Sample verification successful: {data['industry']} has {len(data['keywords'])} keywords")
            else:
