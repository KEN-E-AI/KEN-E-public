...
| AC-2 spot-check | `accounts/*/performance_profiles/...` contains 1,652 docs matching dry-run counts | ✅ Confirmed via the runner's per-source-collection count-match in the verify phase. The runner derives each destination path from `accounts/{extracted_account_id}/performance_profiles` and reads each one — a wrong-path bug would produce mismatched per-account counts before the run could aggregate to 1,652 = 1,652. This is the path-correctness proof for migrated `acc_<hex>` sources; the literal Firestore-console probe in the Linear AC wording was not run (operator-deferred). See Notes for why `collection_group()` was not used (cf. DM-39's precedent). |
...
- **Why direct-path reads (not `collection_group()`) for the spot-check:** The pre-flight collision check and post-test read-back used direct collection-path reads (`accounts/dm40_smoke/performance_profiles`) instead of `collection_group()` queries. The accurate reason: **no `performance_profiles` collection-group composite index is deployed in `deployment/firestore.indexes.json` (against either `(default)` or `analytics`)** — tracked separately in `app/adk/agents/strategy_agent/RUNTIME_WARNINGS_ERRORS.md:229-231,458`. A bare `collection_group("performance_profiles")` iteration without a filter would still have worked (DM-39 did this on the same `analytics` DB for `agent_analytics`); a filtered query (e.g. `.where("account_id", "==", x)`) would not. This run-log used direct-path reads as a deliberate simpler probe. **A future run could add a bare `collection_group("performance_profiles")` iteration on `analytics` as a stronger spot-check; that was not done here and is not a blocker for AC-2 (the verify-phase per-source-collection count-match is the path-correctness proof — see AC table).**
- **Composite-index deliverable scope:** the `performance_profiles` collection-group composite index (`is_bottleneck`, `duration_seconds`, `timestamp`, `__name__`) referenced by `RUNTIME_WARNINGS_ERRORS.md:229-231,458` and `DM-PRD-02 §9 line 147` is **not added by this PR.** DM-40's formal ACs (PRD §6 + Linear) cover data migration only — the index work is tracked separately (operator action: deploy via `terraform apply` of `deployment/firestore.indexes.json` once the index entry is added, analogous to the DM-69/DM-72 pattern for `strategy_audit`). Until the index is deployed, `get_bottlenecks` and any other filtered `collection_group("performance_profiles")` queries will continue to throw `FAILED_PRECONDITION`. This intentional deferral preserves the DM-19/DM-39 precedent of scoping the dev-migration run-log to the data move + smoke confirmation only.
