    ...
    for org_data in ORGANIZATION_DATA:
        if org_data["agency"] and org_data["child_organizations"]:
            ...
            for child_id in org_data["child_organizations"]:
                ...
                logger.info(f"Created PARENT_OF relationship: {parent_id} -> {child_id}")
...
async def verify_migration(db: Neo4jService):
    ...
    for record in list_result:
        logger.info(f"  {record['org_name']}: {', '.join(record['accounts']) if record['accounts'][0] else 'No accounts'}")
...
async def main():
    ...
    try:
        ...
        for org_data in ORGANIZATION_DATA:
            await create_organization(db, org_data.copy())  # Use copy to avoid modifying original
