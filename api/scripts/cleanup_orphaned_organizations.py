...
async def main(dry_run: bool = True):
    ...
    try:
        ...
        logger.info(f"Users with orphaned organizations: {stats['users_with_orphaned_orgs']}")
        logger.info(f"Unique orphaned organizations found: {len(stats['orphaned_orgs_found'])}")
        
        if stats['orphaned_orgs_found']:
            logger.info(f"Orphaned organization IDs: {sorted(stats['orphaned_orgs_found'])}")
