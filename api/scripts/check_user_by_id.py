...
def check_user_by_id(user_id: str):
    ...
    if test_account in all_accessible_accounts:
        ...
        print(
            f"   Role: {account_permissions.get(test_account) or account_level_permissions.get(test_account)}"
        )
    else:
    ...
    if status_docs:
        for doc in status_docs:
            data = doc.to_dict()
            print(
                f"  - {doc.id}: status={data.get('status', 'unknown')}, updated={data.get('updated_at', 'N/A')}"
            )
