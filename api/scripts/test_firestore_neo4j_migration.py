...
async def test_firestore_connection():
    ...
    try:
        ...
        for org in orgs:
            logger.info(f"  - {org.get('organization_name', 'Unknown')} (ID: {org.get('id', 'No ID')})")
...
async def test_sample_migration():
    ...
    try:
        ...
        test_org_id = firestore.create_document("organizations", "test-migration-org", test_org)
        ...
        RETURN org
        """
        result = await neo4j.execute_query(verify_query, {"organization_id": test_org_id})
