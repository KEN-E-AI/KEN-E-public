...
async def test_account_creation():
    ...
    if not AUTH_TOKEN:
        ...
        logger.info(
            "go to Network tab, and look for Authorization header in API calls."
        )
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                if elapsed < 5:
                    logger.info(
                        f"✅ EXCELLENT: Response time under 5 seconds ({elapsed:.2f}s)"
                    )
                elif elapsed < 30:
                    logger.info(
                        f"✅ GOOD: Response time under 30 seconds ({elapsed:.2f}s)"
                    )
                else:
                    logger.warning(
                        f"⚠️ SLOW: Response time over 30 seconds ({elapsed:.2f}s)"
                    )
...
async def main():
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            ...
            if response.status_code != 200:
                logger.error(
                    "API health check failed. Is the API running on port 8000?"
                )
                return
    except Exception as e:
        logger.error(f"Cannot connect to API on localhost:8000: {e}")
        logger.info(
            "Please start the API with: cd api && uv run uvicorn src.kene_api.main:app --reload --host 0.0.0.0 --port 8000"
        )
        return
