...
async def delete_account_cascade(account_id: str, account_name: str, data_region: str):
    ...
    try:
        ...
        try:
            deleted_documents = await storage.delete_account_documents(account_id, data_region)
            cleanup_results["gcs_documents_deleted"] = 1 if deleted_documents else 0
            logger.info(f"  ✅ Deleted GCS documents: {cleanup_results['gcs_documents_deleted']}")
        ...
        DETACH DELETE log
        """
        logs_summary = await db.execute_write_operation(delete_logs_query, {"account_id": account_id})
        ...
        """
        entities_summary = await db.execute_write_operation(delete_entities_query, {"account_id": account_id})
        ...
        DETACH DELETE acc
        """
        account_summary = await db.execute_write_operation(delete_account_query, {"account_id": account_id})
