...
async def test_frontend_history_format():
    ...
    try:
        ...
        history = await client.get_conversation_history(
            user_id=test_user.user_id, session_id=test_conversation.session_id
        )
        ...
        if history and (history.get("messages") or history.get("events")):
            ...
            for index, event in enumerate(events):
                print(f"\n🔍 Event {index + 1}:")
                print(
                    f"   Raw event keys: {list(event.keys()) if isinstance(event, dict) else 'Not a dict'}"
                )
                ...
                if isinstance(event, dict):
                    if event.get("content") and isinstance(event["content"], dict):
                        ...
                        if content_obj.get("parts") and isinstance(
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print(
                "🎯 The frontend should now be able to parse conversation history properly"
            )
        else:
