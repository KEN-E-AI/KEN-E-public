    ...
    try:
        ...
        if sync_resp.status_code == 200:
            ...
            if data["success"]:
                ...
                print(
                    f"  Total holidays in BigQuery: {sync_data['total_holidays_in_bigquery']}"
                )
                print(
                    f"  Existing logs before sync: {sync_data['existing_logs_before_sync']}"
                )
                print(f"  New logs created: {sync_data['new_logs_created']}")
                print(f"  Logs deleted: {sync_data['logs_deleted']}")
                print(
                    f"  Logs protected from deletion: {sync_data['logs_protected_from_deletion']}"
                )
                ...
                if activities_resp.status_code == 200:
                    ...
                    for activity in activities:
                        if activity["id"] == "act_00":
                            ...
                            for log in sorted(
                                logs, key=lambda x: x.get("description", "")
                            ):
                                print(
                                    f"  - {log.get('description', 'Unknown')} ({log.get('start_date', 'Unknown')})"
                                )
                            break
                    else:
