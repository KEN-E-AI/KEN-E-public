...
def main():
    ...
    for template in templates:
        ...
        if has_definition:
            print(f"\n🗑️  {industry_name}")
            print(f"   Removing definition field...")
            
            # Update the document to remove the definition field
            update_data = {
                "definition": DELETE_FIELD
            }
            ...
            if success:
                print(f"   ✓ Definition field removed")
                updated_count += 1
            else:
                print(f"   ✗ Failed to remove definition field")
        else:
            print(f"\n⏭️  {industry_name}")
            print(f"   No definition field to remove")
    
    print("\n" + "=" * 80)
    print(f"✅ Completed: Removed definition field from {updated_count} of {len(templates)} templates")
    return 0
