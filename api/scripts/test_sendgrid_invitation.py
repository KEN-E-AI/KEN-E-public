...
def test_sendgrid_configuration():
    ...
    logger.info(f"SENDGRID_API_KEY env var: {sendgrid_api_key_env[:50]}..." if sendgrid_api_key_env else "NOT SET")
...
if __name__ == "__main__":
    ...
    if response == 'y':
        ...
        if test_email and '@' in test_email:
            ...
            original_send = src.kene_api.email_service.EmailService.send_invitation_email
            
            def send_with_test_email(self, to_email, *args, **kwargs):
                return original_send(self, test_email, *args, **kwargs)
            
            src.kene_api.email_service.EmailService.send_invitation_email = send_with_test_email
