...
class FirestoreToNeo4jMigrator:
        ...
        try:
            ...
            for org in organizations:
                try:
                    ...
                    logger.debug(f"Read organization: {org.get('organization_name', 'Unknown')} ({org.get('organization_id', 'No ID')})")
    ...
    async def check_organization_exists_in_neo4j(self, organization_id: str) -> bool:
        ...
        """
        result = await self.neo4j.execute_query(query, {"organization_id": organization_id})
    ...
    async def create_organization_in_neo4j(self, org_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            params = {
                ...
                "subscription": json.dumps(subscription) if isinstance(subscription, dict) else subscription,
                "billing": json.dumps(billing) if isinstance(billing, dict) else billing,
                "team": json.dumps(team) if isinstance(team, dict) else team
            }
            
            await self.neo4j.execute_write_query(create_query, params)
            logger.info(f"Created organization in Neo4j: {org_data.get('organization_name')} ({organization_id})")
            self.stats['organizations_created'] += 1
            return True
            
        except Exception as e:
            logger.error(f"Failed to create organization {org_data.get('organization_id', 'Unknown')}: {e}")
            self.stats['errors'].append({
    ...
    async def update_organization_in_neo4j(self, org_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if result.get('properties_set', 0) > 0:
                logger.info(f"Updated organization in Neo4j: {org_data.get('organization_name')} ({organization_id})")
            ...
            logger.error(f"Failed to update organization {org_data.get('organization_id', 'Unknown')}: {e}")
            self.stats['errors'].append({
                'organization_id': org_data.get('organization_id', 'Unknown'),
                'error': str(e),
                'phase': 'update'
            ...
            for record in results:
                ...
                for child_id in children:
                    try:
                        ...
                        logger.debug(f"Created PARENT_OF relationship: {parent_id} -> {child_id}")
                        
                    except Exception as e:
                        logger.error(f"Failed to create relationship {parent_id} -> {child_id}: {e}")
                        self.stats['errors'].append({
                            'parent_id': parent_id,
                            'child_id': child_id,
                            'error': str(e),
    ...
    async def migrate_organization(self, org_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if exists:
                ...
                logger.info(f"Organization already exists, updating: {organization_name} ({organization_id})")
                await self.update_organization_in_neo4j(org_data)
            else:
                # Create new organization
                logger.info(f"Creating new organization: {organization_name} ({organization_id})")
    ...
    async def verify_migration(self) -> None:
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"Created from Firestore: {counts['created_from_firestore']}")
                logger.info(f"Updated from Firestore: {counts['updated_from_firestore']}")
            ...
            for org in organizations:
                ...
                logger.info(f"  - {org['name']} ({org['id']}) - Plan: {org['plan']}{agency_str}{children_str}")
    ...
    async def run_migration(self) -> None:
        ...
        try:
            ...
            logger.info(f"Organizations read from Firestore: {self.stats['organizations_read']}")
            logger.info(f"Organizations created in Neo4j: {self.stats['organizations_created']}")
            logger.info(f"Organizations updated in Neo4j: {self.stats['organizations_updated']}")
