...
def check_environment_config():
    ...
    print(
        f"   RECAPTCHA_SITE_KEY: {'✅ Set' if settings.RECAPTCHA_SITE_KEY else '❌ Missing'}"
    )
    print(
        f"   RECAPTCHA_SECRET_KEY: {'✅ Set' if settings.RECAPTCHA_SECRET_KEY else '❌ Missing'}"
    )
...
async def test_recaptcha_service():
    ...
    result = await recaptcha_service.verify_token(
        "invalid-token-12345", expected_action="signin"
    )
    print(f"      Result: {result}")
...
def print_common_issues():
    ...
    print(
        "      - Check that localhost is added to allowed domains in Google reCAPTCHA admin"
    )
    ...
    print(
        "      - Verify the backend can reach www.google.com/recaptcha/api/siteverify"
    )
    print()
