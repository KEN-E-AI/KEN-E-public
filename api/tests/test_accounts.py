...
def test_create_account(mock_neo4j_service):
    ...
    mock_neo4j_service.execute_query.side_effect = [
        ...
        [{  # Return created account
            "acc": {
                "account_id": "test-org-new-account",
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["account_id"] == "test-org-new-account"
...
def test_create_account_duplicate_account_id(mock_neo4j_service):
    """Test creating account when generated account_id already exists."""
    account_data = {
        "account_name": "Test Account",
        "organization_id": "test-org",
        "industry": "Technology",
    ...
    # Mock organization exists check passes, but creation fails due to constraint
    mock_neo4j_service.execute_query.side_effect = [
        [{"exists": True}],  # Organization exists
        Exception("Node with label `Account` and property `account_id` = 'test-account' already exists")
    ]
