...
class TestFirestoreNotificationBatching:
    ...
    async def test_100_accounts_parallel_execution(self, repository):
        ...
        with patch.object(repository, '_fetch_batch', side_effect=mock_fetch_with_delay):
            result = await repository.get_by_account(account_ids)
    ...
    @pytest.mark.asyncio
    async def test_pagination_with_single_batch(self, repository, mock_firestore_client):
    ...
    async def test_pagination_with_multiple_batches(self, repository):
        ...
        with patch.object(repository, '_fetch_batches_parallel') as mock_fetch:
            ...
            result = await repository.get_by_account(
                account_ids, 
                limit=10, 
                offset=10
            )
    ...
    async def test_pagination_boundary_cases(self, repository, mock_firestore_client):
        ...
        result = await repository.get_by_account(
            account_ids, limit=10, offset=5
        )
        assert result == []
        
        # Test offset greater than total count
        result = await repository.get_by_account(
            account_ids, limit=10, offset=100
        )
        assert result == []
        
        # Test limit of 0 (should return empty)
        result = await repository.get_by_account(
            account_ids, limit=0, offset=0
        )
        assert result == []
    ...
    async def test_notifications_sorted_correctly(self, repository):
        ...
        with patch.object(repository, '_fetch_batch') as mock_fetch:
            ...
            for i in range(1, len(result)):
                ...
                actual_diff = datetime.fromisoformat(result[i-1].created_at) - datetime.fromisoformat(result[i].created_at)
    ...
    @pytest.mark.asyncio
    async def test_duplicate_notifications_handled(self, repository, mock_firestore_client):
    ...
    async def test_concurrent_modifications(self, repository):
        ...
        with patch.object(repository, '_fetch_batch', side_effect=mock_fetch_with_modification):
            result = await repository.get_by_account(account_ids)
