...
from src.kene_api.routers.accounts import update_account, _sync_holiday_activity_logs_for_account
...
class TestRegionChangeSync:
    ...
    async def test_region_change_us_to_au_removes_us_holidays(
        ...
        delete_calls = [
            call for call in mock_neo4j_service.execute_write_query.call_args_list
    ...
    async def test_protected_logs_not_deleted(
        ...
        delete_params = delete_calls[0][0][1]
        assert "log_us_memorial_day" in delete_params["log_ids"]
        assert "log_us_presidents_day" not in delete_params["log_ids"]  # Protected, not deleted
