...
class TestCacheParameterized:
    ...
    @pytest.mark.parametrize("max_size,ttl_seconds,expected_caching", [
        (0, 60, False),  # No caching when max_size is 0
        (10, 0, False),  # No caching when TTL is 0
        (10, 60, True),  # Normal caching
    ])
    def test_cache_configuration_scenarios(self, max_size, ttl_seconds, expected_caching):
    ...
    @pytest.mark.parametrize("num_threads,num_operations", [
        (5, 10),
        (10, 20),
        (20, 50),
    ])
    ...
    @pytest.mark.parametrize("pattern,keys_to_set,expected_removed", [
        ("industry:", ["industry:a", "industry:b", "id:1"], ["industry:a", "industry:b"]),
        ("test", ["test1", "test2", "other"], ["test1", "test2"]),
        ("", ["key1", "key2", "key3"], ["key1", "key2", "key3"]),  # Empty pattern matches all
    ])
    def test_cache_pattern_invalidation_scenarios(self, pattern, keys_to_set, expected_removed):
