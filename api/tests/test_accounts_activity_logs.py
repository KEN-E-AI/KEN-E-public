...
async def test_create_initial_activity_logs_success():
    ...
    # Mock act_00 exists
    mock_neo4j.execute_query = AsyncMock(return_value=[{"a": {"activity_id": "act_00"}}])
    
    # Mock holiday data from BigQuery
    mock_bigquery.query_holiday_activities = Mock(return_value=[
        {"description": "New Year's Day", "start_date": "2024-01-01", "end_date": "2024-01-01"},
        {"description": "Christmas Day", "start_date": "2024-12-25", "end_date": "2024-12-25"},
    ])
    ...
    assert result == 2
    mock_bigquery.query_holiday_activities.assert_called_once_with("test-project", ["AU", "CA"])
...
async def test_create_initial_activity_logs_exception_handling():
    ...
    mock_bigquery.query_holiday_activities = Mock(side_effect=Exception("BigQuery error"))
...
def test_create_account_with_regions(mock_neo4j_service, mock_firestore_service, mock_bigquery_service):
    ...
    mock_bigquery_service.query_holiday_activities.return_value = [
        {"description": "New Year", "start_date": "2024-01-01", "end_date": "2024-01-01"}
    ]
...
def test_create_account_without_regions(mock_neo4j_service, mock_firestore_service, mock_bigquery_service):
