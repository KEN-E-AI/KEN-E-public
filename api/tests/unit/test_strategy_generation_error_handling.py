...
class TestStrategyGenerationErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_empty_agent_response_marks_account_failed(
        ...
        with patch("src.kene_api.tasks.strategy_tasks.vertexai") as mock_vertexai:
            with patch("src.kene_api.tasks.strategy_tasks.agent_engines") as mock_engines:
                with patch("src.kene_api.tasks.strategy_tasks.update_account_setup_status") as mock_update_status:
                    with patch("src.kene_api.tasks.strategy_tasks.get_firestore_service"):
                        with patch("src.kene_api.tasks.strategy_tasks.get_neo4j_service"):
                            ...
                            await trigger_strategy_generation(
                                ...
                                uploaded_document_urls=mock_account_data["uploaded_document_urls"],
    ...
    @pytest.mark.asyncio
    async def test_agent_response_with_no_chunks_marks_failed(
        self, mock_account_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_agent_iteration_error_handles_gracefully(
        self, mock_account_data
    ):
            ...
            with patch("src.kene_api.tasks.strategy_tasks.agent_engines") as mock_engines:
                with patch("src.kene_api.tasks.strategy_tasks.update_account_setup_status") as mock_update_status:
                    with patch("src.kene_api.tasks.strategy_tasks.verify_strategy_documents_created") as mock_verify:
                        with patch("src.kene_api.tasks.strategy_tasks.get_firestore_service") as mock_get_firestore:
                            with patch("src.kene_api.tasks.strategy_tasks.get_neo4j_service") as mock_get_neo4j:
                                # Setup mocks
                                ...
                                mock_agent.stream_query.return_value = [
                                    {"content": {"parts": [{"text": "Strategy content"}]}}
                                ]
                                mock_engines.get.return_value = mock_agent
                                ...
                                with patch("src.kene_api.tasks.strategy_tasks.asyncio.sleep") as mock_sleep:
                                    mock_sleep.side_effect = [None] * 3  # Allow 3 iterations
    ...
    @pytest.mark.asyncio
    async def test_agent_response_timeout_handling(
        self, mock_account_data
    ):
        ...
        with patch("src.kene_api.tasks.strategy_tasks.vertexai") as mock_vertexai:
            with patch("src.kene_api.tasks.strategy_tasks.agent_engines") as mock_engines:
                with patch("src.kene_api.tasks.strategy_tasks.update_account_setup_status") as mock_update_status:
                    with patch("src.kene_api.tasks.strategy_tasks.get_firestore_service") as mock_get_firestore:
                        with patch("src.kene_api.tasks.strategy_tasks.get_neo4j_service") as mock_get_neo4j:
                            with patch("src.kene_api.tasks.strategy_tasks.asyncio.get_event_loop") as mock_loop:
                                # Setup mocks
                                mock_get_firestore.return_value = mock_firestore_service
                                mock_get_neo4j.return_value = mock_neo4j_service
                                ...
                                class InfiniteIterator:
                                    def __iter__(self):
                                        return self
                                    def __next__(self):
                                        return {"content": {"parts": [{"text": "chunk"}]}}
                                
                                mock_agent = MagicMock()
                                mock_agent.stream_query.return_value = InfiniteIterator()
                                mock_engines.get.return_value = mock_agent
    ...
    @pytest.mark.asyncio
    async def test_successful_strategy_generation_flow(
        self, mock_account_data
    ):
                    ...
                    with patch("src.kene_api.tasks.strategy_tasks.verify_strategy_documents_created") as mock_verify:
                        with patch("src.kene_api.tasks.strategy_tasks.get_firestore_service") as mock_get_firestore:
                            with patch("src.kene_api.tasks.strategy_tasks.get_neo4j_service") as mock_get_neo4j:
                                ...
                                mock_agent = MagicMock()
                                mock_agent.stream_query.return_value = [
                                    {"content": {"parts": [{"text": "Business strategy content"}]}},
                                    {"content": {"parts": [{"text": "Marketing strategy content"}]}},
                                    {"content": {"parts": [{"text": "Customer strategy content"}]}},
                                ]
                                mock_engines.get.return_value = mock_agent
    ...
    @pytest.mark.asyncio
    async def test_agent_call_failure_raises_exception(
        self, mock_account_data
    ):
            ...
            with patch("src.kene_api.tasks.strategy_tasks.agent_engines") as mock_engines:
                with patch("src.kene_api.tasks.strategy_tasks.get_firestore_service") as mock_get_firestore:
                    with patch("src.kene_api.tasks.strategy_tasks.get_neo4j_service") as mock_get_neo4j:
                        ...
                        mock_agent = MagicMock()
                        mock_agent.stream_query.side_effect = Exception("Agent unavailable")
                        mock_engines.get.return_value = mock_agent
