...
class TestCachedNotificationRepository:
    ...
    def cached_repository(self, base_repository):
        ...
        return CachedNotificationRepository(base_repository, cache_ttl=1)  # 1 second TTL
    ...
    async def test_cache_key_generation(self, cached_repository):
        ...
        key1 = cached_repository._make_count_cache_key(user_id, ["acc_123", "acc_456", "acc_789"])
        key2 = cached_repository._make_count_cache_key(user_id, ["acc_789", "acc_123", "acc_456"])
    ...
    async def test_batch_operations_invalidate_cache(self, cached_repository):
        ...
        assert not any(k.startswith("user_1:") for k in cached_repository._unread_count_cache)
        
        # user_2's cache should still exist
        assert any(k.startswith("user_2:") for k in cached_repository._unread_count_cache)