...
class TestStorageService:
    ...
    def mock_storage_client(self):
        ...
        with patch("src.kene_api.services.storage_service.storage.Client") as mock_client:
            yield mock_client
    ...
    @pytest.mark.asyncio
    async def test_ensure_account_folder_creates_placeholder(self, storage_service, mock_storage_client):
        ...
        storage_service.ensure_bucket_exists = AsyncMock(return_value=("test-bucket", "us-central1"))
    ...
    @pytest.mark.asyncio
    async def test_ensure_account_folder_already_exists(self, storage_service, mock_storage_client):
    ...
    @pytest.mark.asyncio
    async def test_ensure_account_folder_handles_errors(self, storage_service, mock_storage_client):
        """Test that ensure_account_folder handles errors gracefully."""
        # Setup mocks to raise an exception
        mock_storage_client.return_value.get_bucket.side_effect = Exception("Storage error")
    ...
    @pytest.mark.asyncio
    async def test_ensure_account_folder_different_regions(self, storage_service, mock_storage_client):
        ...
        storage_service.ensure_bucket_exists = AsyncMock(return_value=("test-bucket-eu", "europe-west1"))
