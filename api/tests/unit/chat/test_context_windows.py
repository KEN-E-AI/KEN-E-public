...
class TestLintScript:
    ...
    def test_lint_fails_on_unregistered_model(self, tmp_path: Path) -> None:
        """Planting a file with model='not-real-model' in app/adk/agents/ must
        cause the lint to exit non-zero and name the unregistered model."""
        fixture_dir = REPO_ROOT / "app" / "adk" / "agents" / "_lint_fixture"
        fixture_file = fixture_dir / "agent.py"

        fixture_dir.mkdir(parents=True, exist_ok=True)
        (fixture_dir / "__init__.py").write_text("")
        fixture_file.write_text(
            "# lint fixture — auto-generated by test; do not commit\n"
            "class _FakeAgent:\n"
            "    pass\n"
            "\n"
        ...
        try:
            ...
            # Always clean up the fixture so subsequent lint runs are unaffected
