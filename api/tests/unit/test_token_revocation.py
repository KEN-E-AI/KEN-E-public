...
class TestTokenRevocationService:
    ...
    def revocation_service(self):
        ...
        with mock.patch("src.kene_api.auth.token_revocation.get_redis_service") as mock_redis:
    ...
    async def test_revoke_token(self, revocation_service):
        ...
        # Mock dependencies
        with mock.patch("src.kene_api.auth.token_revocation.get_firestore_service") as mock_firestore:
            with mock.patch("src.kene_api.auth.token_revocation.get_audit_logger") as mock_audit:
                # Setup mocks
    ...
    async def test_is_token_revoked_all_tokens(self, revocation_service):
        ...
        result = await revocation_service.is_token_revoked("token123", "user456", issued_at)
        assert result is True
