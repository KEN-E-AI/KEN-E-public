...
from src.kene_api.models.monitoring_models import ConceptOption, ConceptType, ConceptReference
...
class TestConceptDisambiguationService:
    ...
    async def test_search_wikipedia_success(self, service, mock_http_client):
        ...
        search_response.json.return_value = [
            ...
            ["Apple Inc.", "Apple"],
            ["", ""],
            ["https://en.wikipedia.org/wiki/Apple_Inc.", "https://en.wikipedia.org/wiki/Apple"]
        ]
    ...
    async def test_map_wikidata_type(self, service):
        ...
        assert service._map_wikidata_type("American technology company") == ConceptType.COMPANY
        assert service._map_wikidata_type("city in California") == ConceptType.LOCATION
        assert service._map_wikidata_type("American businessperson") == ConceptType.PERSON
    ...
    async def test_determine_source_type(self, service):
        ...
        assert service._determine_source_type("https://en.wikipedia.org/wiki/Apple") == "wikipedia"
        assert service._determine_source_type("https://www.wikidata.org/wiki/Q312") == "wikidata"
        assert service._determine_source_type("https://www.reuters.com/article/123") == "news_source"
        assert service._determine_source_type("https://www.britannica.com/topic/apple") == "encyclopedia"
        assert service._determine_source_type("https://www.irs.gov/forms") == "government"
